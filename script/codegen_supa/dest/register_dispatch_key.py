...
from torchgen.dest.register_dispatch_key import gen_registration_helpers  # noqa
from codegen_supa import supa_utils
    ...
    def __call__(self, f: Union[NativeFunctionsGroup, NativeFunction]) -> List[str]:
        if isinstance(f, NativeFunctionsGroup):
            ...
            for func in g.functions():
                native_metadata = getattr(func, "native_metadata", None)
                perf = perf or getattr(func, "perf", False)
                if native_metadata is not None:
                    break
            for func in g.functions():
                native_metadata_tmp = getattr(func, "native_metadata", None)
                if native_metadata_tmp is None:
                    object.__setattr__(func, "native_metadata", native_metadata)
                    object.__setattr__(func, "perf", perf)
...
"""
    ...
    def gen_structured(self, g: NativeFunctionsGroup) -> List[str]:
        ...
        elif metadata is None or not metadata.structured:  # metadata is None for autograd case
            return list(mapMaybe(lambda f: self.gen_unstructured(f, g), g.functions()))
...
"""
                ...
                if self.class_method_name is None:
                    impl_name = f"{metadata.cpp_namespace}::supa::{metadata.kernel}"
                else:
                    impl_name = f"{metadata.cpp_namespace}::supa::{self.class_method_name}::{metadata.kernel}"
                ...
                profiler_recorder = "  torch_supa::profiler::SUPARecordFunction recorder;"
...
"""
                ...
                if (
                    f.manual_kernel_registration
                    or self.skip_dispatcher_op_registration
                    or getattr(f, "is_custom_supa", False)
                ):
                    return None
                else:
...
"""
        ...
        if maybe_create_proxy:
            create_proxy = """
auto maybe_proxy = supa::maybe_create_proxy(out, sizes, strides, options);
...
outputs_[output_idx] = supa::create_out(sizes, strides, options);"""
        elif k is SchemaKind.inplace:
            return f"""{maybe_set_guard_line}
const auto& out = outputs_[output_idx].get();
supa::check_inplace(out, sizes, options);
{create_proxy}"""
        elif k is SchemaKind.out:
            return f"""{maybe_set_guard_line}
const auto& out = outputs_[output_idx].get();
supa::resize_out(out, sizes, strides, options);
{create_proxy}"""
    ...
    def gen_class(
        ...
        *,
        ...
        if self.backend_index.dispatch_key == DispatchKey.CUDA:
            ...
            guard_field = "c10::supa::OptionalSUPAGuard guard_;"
    ...
    def gen_one(self, f: NativeFunction) -> Optional[str]:
        ...
        if getattr(f, "supa_kernel", False):
            return None
...
"""
            ...
            if self.backend_index.dispatch_key is DispatchKey.Meta:
                ...
                parent_class = f"at::meta::structured_{meta.name(self.g)}"
            elif self.backend_index.dispatch_key in supa_utils.CustomYamlProcessor.supported_dispatch_keys:
                metadata = self.backend_index.get_kernel(self.g)
                assert metadata is not None
                class_name = f"structured_{meta.name(self.g)}_{k.name}"
                parent_class = f"{metadata.cpp_namespace}::supa::structured_{meta.name(self.g)}"
            else:
                metadata = self.backend_index.get_kernel(self.g)
                assert metadata is not None
                class_name = f"structured_{meta.name(self.g)}_{k.name}"
            ...
            # supa single implements.
            _sig = self.wrapper_kernel_sig(f)
            _kernel_sig = kernel_signature(f, self.backend_index)
            args_exprs_str = ", ".join(
                e.expr for e in translate(_sig.arguments(), _kernel_sig.arguments(), method=False)
            )
            ...
            if self.backend_index.dispatch_key == DispatchKey.CompositeExplicitAutogradNonFunctional:
                ...
                if self.g.has_supa_structs:
                    supa_calling = f"""op.impl_supa({impl_exprs});"""
                    fmt = "if (torch_supa::utils::EnvConfig::IsEnableNativeOP()) {{\n   {}\n  }} else {{\n   {}\n  }}"
                    if getattr(f, "perf", False):
...
"""
            ...
            if getattr(f, "is_custom_supa", False):
                return None
