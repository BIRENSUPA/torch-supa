...
// Some stateful GPU libraries, such as suDNN, suBLAS, use handles to store
...
struct DeviceThreadHandlePool : public std::enable_shared_from_this<DeviceThreadHandlePool<Handle_t, Create, Destroy>> {
  struct Handle {
    Handle_t handle;
    explicit Handle(bool create = false) : handle(nullptr) {
      if (create) {
        Create(&handle);
      }
    }
    ...
    // want the destructors of temporaries to call sudnnDestroy on the handle.
  ...
  class PoolWindow {
   public:
    explicit PoolWindow(std::shared_ptr<DeviceThreadHandlePool> parent) : weak_parent(std::move(parent)) {}
    PoolWindow(const PoolWindow&) = delete;
    PoolWindow(PoolWindow&&) = delete;
    PoolWindow& operator=(const PoolWindow&) = delete;
    PoolWindow& operator=(PoolWindow&&) = delete;
    ~PoolWindow() {
      release();
    }
    ...
    Handle_t reserve(int device) {
      ...
      if (!parent->available_handles[device].empty()) {
    ...
    void release() {
      if (!my_handles.empty()) {
        ...
        if (!parent) {
          ...
          // supa context itself may be invalid, so we must leak the handles.
          return;
        }
