...
#define TORCH_USE_SUPA_DSA

/// Number of assertion failure messages we can store. If this is too small
/// threads will fail silently.
constexpr int C10_SUPA_DSA_ASSERTION_COUNT = 10;
constexpr int C10_SUPA_DSA_MAX_STR_LEN = 512;
...
struct DeviceAssertionData {
  ...
  char assertion_msg[C10_SUPA_DSA_MAX_STR_LEN]{};
  /// File the assertion was in
  // NOLINTNEXTLINE(*-c-arrays)
  char filename[C10_SUPA_DSA_MAX_STR_LEN]{};
  /// Name of the function the assertion was in
  // NOLINTNEXTLINE(*-c-arrays)
  char function_name[C10_SUPA_DSA_MAX_STR_LEN]{};
...
struct DeviceAssertionsData {
  ...
  // NOLINTNEXTLINE(*-c-arrays)
  DeviceAssertionData assertions[C10_SUPA_DSA_ASSERTION_COUNT]{};
};
...
struct SUPAKernelLaunchInfo {
  ...
  /// SUPAKernelLaunchRegistry::gather_launch_stacktrace is True
...
/// occurred SUPAKernelLaunchRegistry is used as a singleton
class C10_SUPA_API SUPAKernelLaunchRegistry {
 private:
  ...
  std::vector<std::unique_ptr<DeviceAssertionsData, void (*)(DeviceAssertionsData*)>> uvm_assertions;
  /// A single circular buffer holds information about every kernel launch the
  /// process makes across all devices.
  std::vector<SUPAKernelLaunchInfo> kernel_launches;
  static bool check_env_for_enable_launch_stacktracing();
  static bool check_env_for_dsa_enabled();

 public:
  SUPAKernelLaunchRegistry();
  ...
  std::pair<std::vector<DeviceAssertionsData>, std::vector<SUPAKernelLaunchInfo>> snapshot() const;
  ...
  static SUPAKernelLaunchRegistry& get_singleton_ref();
...
C10_SUPA_API std::string c10_retrieve_device_side_assertion_info();
...
#define TORCH_DSA_KERNEL_ARGS \
  [[maybe_unused]] c10::supa::DeviceAssertionsData *const assertions_data, [[maybe_unused]] uint32_t assertion_caller_id
