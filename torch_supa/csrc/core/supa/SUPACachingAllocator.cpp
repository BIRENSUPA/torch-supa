...
namespace supa::SUPACachingAllocator {

C10_DEFINE_REGISTRY(FreeSUPAMemoryCallbacksRegistry, FreeMemoryCallback);
...
enum ShareableHandleType : char { SHAREABLE_SUPA_MALLOC = 'c', SHAREABLE_SUPA_EXPANDABLE_SEGMENT = 'e' };

namespace {

using stream_set = ska::flat_hash_set<c10::supa::SUPAStream>;
...
#if TORCH_VER < TORCH_2_5_0
inline void update_stat(Stat& stat, int64_t amount) {
  stat.current += amount;
  TORCH_CHECK(stat.current >= 0, "Negative tracked stat in SUPA allocator (likely logic error).");
...
void decrease_stat_array(StatArray& stat_array, size_t amount, const StatTypes& stat_types) {
  for_each_selected_stat_type(
      stat_types, [&stat_array, amount](size_t stat_type) { stat_array[stat_type].decrease(amount); });
}
#endif

inline void reset_accumulated_stat(Stat& stat) noexcept {
  stat.allocated = 0;
  stat.freed = 0;
}

inline void reset_peak_stat(Stat& stat) noexcept {
...
#if TORCH_VER < TORCH_2_5_0
inline void update_stat_array(StatArray& stat_array, int64_t amount, const StatTypes& stat_types) {
  if (amount == 0) {
    return;
  }
  for (size_t stat_type = 0, size = stat_types.size(); stat_type < size; ++stat_type) {
    if (stat_types[stat_type]) {
...
struct Block {
  ...
  supaStream_t stream; // allocation stream
  ...
  int event_count{0}; // number of outstanding SUPA events
  ...
  // this records the frame information when supaMalloc was called
  ...
  Block(c10::DeviceIndex device, supaStream_t stream, size_t size, BlockPool* pool, void* ptr)
      : device(device), stream(stream), size(size), requested_size(0), pool(pool), ptr(ptr) {}

  // constructor for search key
  Block(c10::DeviceIndex device, supaStream_t stream, size_t size)
...
std::pair<std::set<Block*, Comparison>::iterator, bool> BlockPool::insert_into_blocks(Block* block) {
...
use supa's low-level memory APIs, which are similar to mmap, to extend the
...
*/
struct ExpandableSegment {
  ExpandableSegment(c10::DeviceIndex device, supaStream_t stream, size_t size, std::vector<c10::DeviceIndex> peers)
      : device_(device),
        stream_(stream),
    ...
    supaDeviceProp prop{};
    C10_SUPA_CHECK(supaGetDeviceProperties(&prop, device_));
    // we allocate enough address space for 1 1/8 the total memory on the GPU.
    // This allows for some cases where we have to unmap pages earlier in the
    // segment to put them at the end.
    max_handles_ = numSegments(prop.totalGlobalMem + prop.totalGlobalMem / 8);
    C10_SUPA_DRIVER_CHECK(suMemAddressReserve(&ptr_, segment_size_ * max_handles_, 0ULL, 0, 0ULL));
  }
  ...
  SegmentRange map(SegmentRange range) {
    ...
    for (auto i : c10::irange(begin, end)) {
      ...
      SUmemGenericAllocationHandle handle = 0;
      ...
      auto status = suMemCreate(&handle, segment_size_, &prop, 0);
      if (status == SUPA_ERROR_OUT_OF_MEMORY) {
        for (auto j : c10::irange(begin, i)) {
          auto h = handles_.at(j).value().handle;
          handles_.at(j) = c10::nullopt;
          C10_SUPA_DRIVER_CHECK(suMemRelease(h));
        }
        trimHandles();
        return rangeFromHandles(begin, begin);
      }
      C10_SUPA_DRIVER_CHECK(status);
      handles_.at(i) = Handle{handle, std::nullopt};
    }
    for (auto i : c10::irange(begin, end)) {
      C10_SUPA_DRIVER_CHECK(suMemMap(ptr_ + i * segment_size_, segment_size_, 0, handles_.at(i).value().handle, 0ULL));
    }
  ...
  SegmentRange share(SegmentRange range, std::ostream& buf) {
    ...
    for (auto i : c10::irange(begin, end)) {
      ...
      if (!handle.fd) {
        int fd = 0;
        // C10_SUPA_DRIVER_CHECK(DriverAPI::get()->suMemExportToShareableHandle(
        C10_SUPA_DRIVER_CHECK(
            suMemExportToShareableHandle(&fd, handle.handle, SU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR, 0));
        handle.fd = fd;
      }
      int fd = *handle.fd;
  ...
  static std::unique_ptr<ExpandableSegment> fromShared(
    ...
    auto segment = std::make_unique<ExpandableSegment>(
        device, c10::supa::getCurrentSUPAStream(), header.num_handles * header.segment_size, peers);
...
#define SYS_pidfd_getfd 438
    ...
    for (auto i : c10::irange(header.num_handles)) {
      ...
      if (myfd == -1) {
        ...
        for (auto& h : segment->handles_) {
          C10_SUPA_DRIVER_CHECK(suMemRelease(h.value().handle));
          h = std::nullopt;
        }
        TORCH_CHECK(
            err != ENOSYS,
      ...
      SUmemGenericAllocationHandle handle = 0;
      C10_SUPA_DRIVER_CHECK(suMemImportFromShareableHandle(
          &handle,
          // NOLINTNEXTLINE(performance-no-int-to-ptr)
          (void*)(uintptr_t)myfd,
          SU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR));
      close((int)myfd);
  ...
  void addPeer(c10::DeviceIndex device) {
    ...
    forEachAllocatedRange([&](size_t begin, size_t end) { setAccess(device, begin, end); });
  }

  ~ExpandableSegment() {
    forEachAllocatedRange([&](size_t begin, size_t end) { unmapHandles(begin, end); });
    C10_SUPA_DRIVER_CHECK(suMemAddressFree(ptr_, segment_size_ * max_handles_));
  }

 private:
  void setAccess(c10::DeviceIndex device, size_t begin, size_t end) const {
    SUmemAccessDesc desc;
    desc.location.type = SU_MEM_LOCATION_TYPE_DEVICE;
    desc.location.id = static_cast<int>(device);
    desc.flags = SU_MEM_ACCESS_FLAGS_PROT_READWRITE;
    C10_SUPA_DRIVER_CHECK(suMemSetAccess(ptr_ + begin * segment_size_, (end - begin) * segment_size_, &desc, 1));
  }
  ...
  void unmapHandles(size_t begin, size_t end) {
    // note: unlike supaFree, MemUnmap and MemRelease do
    // not appear to synchronize in all cases, so we have to wait for the
    // stream to finish before this memory is truly free.

    // cannot call c10::supa::stream_synchronize because
    // it might grab the GIL which can lead to a deadlock
    // Locking order must be GIL -> Allocator Lock
    C10_SUPA_CHECK(supaStreamSynchronize(stream_));
    for (auto i : c10::irange(begin, end)) {
      const SUmemGenericAllocationHandle h = handles_.at(i).value().handle;
      handles_.at(i) = c10::nullopt;
      C10_SUPA_DRIVER_CHECK(suMemUnmap(ptr_ + segment_size_ * i, segment_size_));
      C10_SUPA_DRIVER_CHECK(suMemRelease(h));
    }
    trimHandles();
  }
  ...
  struct Handle {
    SUmemGenericAllocationHandle handle;
    std::optional<int> fd;
  };
  ...
  // std::vector<c10::optional<SUmemGenericAllocationHandle>> handles_;
  std::vector<c10::optional<Handle>> handles_;
...
struct PrivatePoolState : AllocatorState {
  // omitting use_count, and sudaMalloc_count as they remain the same
...
class EventPool {
 public:
  using Event = std::unique_ptr<supaEvent_t, std::function<void(supaEvent_t*)>>;
  // TODO: Explicit device count
  EventPool() : pools_(at::supa::device_count()) {}

  Event get(c10::DeviceIndex device) {
    TORCH_INTERNAL_ASSERT(0 <= device);
    TORCH_INTERNAL_ASSERT(device < static_cast<int>(pools_.size()));
    auto& pool = pools_[device];
    auto destructor = [&pool](supaEvent_t* event) {
      std::lock_guard<std::mutex> g(pool.mutex_);
      pool.event_pool_.push_back(std::unique_ptr<supaEvent_t>(event));
    };
    ...
    // destruction.
    auto new_ptr = std::make_unique<supaEvent_t>();
    C10_SUPA_CHECK(supaEventCreateWithFlags(new_ptr.get(), supaEventDisableTiming));
 ...
 private:
  struct PerDevicePool {
    ...
    std::vector<std::unique_ptr<supaEvent_t>> event_pool_;
  };
...
// SUPA graphs helper
struct PrivatePool {
  PrivatePool(MempoolId_t id, std::shared_ptr<SUPAAllocator> allocator = nullptr)
      : id(std::move(id)),
  ...
  int use_count{1};
  // Number of unfreed supaMallocs made for this pool. When use_count and
  // supaMalloc_count drop to zero, we can delete this PrivatePool from
  // graph_pools.
  int supaMalloc_count{0};
  // Instead of maintaining private BlockPools here, I could stuff all blocks
  // (private or no) into the top-level large_blocks and small_blocks, and
  // distinguish private blocks by adding a "pool id" check above the stream
  // check in BlockComparator. BlockComparator is performance- critical though,
  // I'd rather not add more logic to it.
  std::shared_ptr<SUPAAllocator> allocator_;
  BlockPool large_blocks;
  BlockPool small_blocks;

 public:
  SUPAAllocator* allocator() {
...
supaError_t allocPrimitive(void** ptr, size_t size, AllocParams& p) {
  if (p.pool->owner_PrivatePool && p.pool->owner_PrivatePool->allocator()) {
    *ptr = p.pool->owner_PrivatePool->allocator()->raw_alloc(size);
    return *ptr ? supaSuccess : supaErrorMemoryAllocation;
  }
  return C10_SUPA_ERROR_HANDLED(supaMalloc(ptr, size));
}

supaError_t supaMallocMaybeCapturing(void** ptr, size_t size, AllocParams& p) {
  if (c10::supa::currentStreamCaptureStatusMayInitCtx() == supaStreamCaptureStatusNone) {
    return allocPrimitive(ptr, size, p);
  }
  // It's ok to capture supaMallocs, as long as we never supaFree those
  // addresses before replay.
  // Capturing supaMalloc behaves nicely: it gives the graph new VA,
  // but is ignored (won't leakily allocate new memory) in replays.
  at::supa::SUPAStreamCaptureModeGuard g{supaStreamCaptureModeRelaxed};
  return allocPrimitive(ptr, size, p);
}
...
class DeviceCachingAllocator {
 private:
  ...
  // supaStreamGetCaptureInfo in the hot path.
  std::vector<std::pair<MempoolId_t, std::function<bool(supaStream_t)>>> captures_underway;
  ...
  // outstanding supa events
  ska::flat_hash_map<supa::SUPAStream, std::deque<std::pair<EventPool::Event, Block*>>> supa_events;
  ...
  // Members specific to supa graphs

  // Private pools for supa graphs
  ...
  // was used while supagraph capturing
  std::unordered_map<Block*, stream_set> block_to_supagraph_stream_uses;
 ...
 public:
  DeviceCachingAllocator()
    ...
    stats.max_split_size = static_cast<int64_t>(SUPAAllocatorConfig::max_split_size());
  }
  ...
  Block* malloc(c10::DeviceIndex device, size_t orig_size, supaStream_t stream) {
    ...
    if (C10_LIKELY(captures_underway.empty())) {
      ...
      // A. process_events involves supaEventQueries, illegal during SUPA graph
      //    capture.
    ...
    if (!block_found) {
      ...
      if (C10_UNLIKELY(set_fraction && SUPAAllocatorConfig::garbage_collection_threshold() > 0.0)) {
        garbage_collect_cached_blocks(context);
      }
      // Attempt allocate
      // WARNING: alloc_block may release the allocator lock when calling
      // supaMalloc. So far this function has not modified allocator state, but
      ...
      block_found = alloc_block(params, false, context, lock)
          ...
          || (C10_LIKELY(captures_underway.empty()) && release_cached_blocks(context, {0, 0}) &&
              alloc_block(params, true, context, lock));
    }

    if (!block_found) {
      // For any error code other than supaErrorMemoryAllocation,
      // alloc_block should have thrown an exception already.
      TORCH_INTERNAL_ASSERT(params.err == supaErrorMemoryAllocation);
      ...
      record_trace(
          TraceEntry::OOM,
          static_cast<int64_t>(device_free),
          params.size(),
          params.stream(),
          params.device(),
      ...
      auto allocated_bytes = stats.allocated_bytes[static_cast<size_t>(StatType::AGGREGATE)].current;
      auto reserved_bytes = stats.reserved_bytes[static_cast<size_t>(StatType::AGGREGATE)].current;
      ...
      for (const auto& obs : observers_local) {
        obs(device,
            ...
            static_cast<int64_t>(set_fraction ? allowed_memory_maximum : device_total),
            static_cast<int64_t>(device_free));
      }
      ...
      //                      caching allocator
      // "free": free memory as reported by the SUPA API
      ...
      TORCH_CHECK_WITH(
          OutOfMemoryError,
          false,
          "SUPA out of memory. Tried to allocate ",
  ...
  Block* alloc_found_block(
    ...
    TORCH_INTERNAL_ASSERT(params.err == supaSuccess && params.block != nullptr && params.block->ptr != nullptr);
    ...
    if (split_remainder) {
      ...
      block = new Block(device, stream, size, pool, block->ptr); // NOLINT(cppcoreguidelines-owning-memory)
...
#if TORCH_VER >= TORCH_2_5_0
        decrease_stat_array(stats.inactive_split_bytes, block->size, params.stat_types);
#else
    ...
    if (block->size >= SUPAAllocatorConfig::max_split_size()) {
#if TORCH_VER >= TORCH_2_5_0
  ...
  void free(Block* block) {
    std::shared_ptr<GatheredContext> context = maybeGatherContext(RecordContext::ALL);
    ...
    if (!block->stream_uses.empty()) {
      if (C10_UNLIKELY(!captures_underway.empty())) {
        // It's forbidden to supaEventQuery an event recorded during SUPA graph
  ...
  void* getBaseAllocation(Block* block, size_t* outSize) {
    ...
    TORCH_CHECK(
        ...
        "torch.supa.memory._set_allocator_settings('expandable_"
        "segments:False')");
    while (block->prev) {
      block = block->prev;
    }
  ...
  ShareableHandle shareIpcHandle(Block* block) {
    ...
    if (!block->expandable_segment_) {
      ss.put(SHAREABLE_SUPA_MALLOC);
      Block* base_block = block;
      while (base_block->prev) {
        base_block = base_block->prev;
      }
      offset = (char*)block->ptr - (char*)base_block->ptr;
      supaIpcMemHandle_t handle;
      C10_SUPA_CHECK(supaIpcGetMemHandle(&handle, base_block->ptr));
      ss.write((char*)&handle, SUPA_IPC_HANDLE_SIZE);
    } else {
      ss.put(SHAREABLE_SUPA_EXPANDABLE_SEGMENT);
      auto full_range = block->expandable_segment_->share(SegmentRange(block->ptr, block->size), ss);
      offset = (char*)block->ptr - (char*)full_range.ptr;
    }
    return ShareableHandle{offset, ss.str()};
  }

  void recordStream(Block* block, supa::SUPAStream stream) {
    std::lock_guard<std::recursive_mutex> lock(mutex);
  ...
  double getMemoryFraction() const {
    ...
    size_t device_total = 0;
    C10_SUPA_CHECK(supaMemGetInfo(&device_free, &device_total));
    return static_cast<double>(allowed_memory_maximum) / static_cast<double>(device_total);
  }
  ...
  void setMemoryFraction(double fraction) {
    ...
    size_t device_total = 0;
    C10_SUPA_CHECK(supaMemGetInfo(&device_free, &device_total));
    allowed_memory_maximum = static_cast<size_t>(fraction * static_cast<double>(device_total));
    set_fraction = true;
  }
  ...
  std::unique_ptr<PrivatePoolState> getCheckpointState(MempoolId_t id) {
    ...
    if (graph_pools_freeable.count(id)) {
  ...
  void setSegmentStateToCheckpoint(
    ...
    for (size_t i = 0; i < segment_len; ++i) {
      ...
      AllocParams params(block_state.device, block_state.size, block_state.stream, &pool, block_state.size, stats);
      ...
      // the returned value
      curr_block = alloc_found_block(std::move(params), block_state.size, context, split);
   ...
   *
   * Refer above to Note [Interaction with SUPA graph capture]. Allocations made
   ...
   * will not free its blocks to the main caching allocator until supa graph use
   ...
   * successive supa graphs, can share a memory pool because after a supa graph
   ...
   * captured with supa graphs. In the diagram below, we have a tree with four
  ...
  RestoreResult setCheckpointPoolState(PrivatePoolState& pps) {
    ...
    std::shared_ptr<GatheredContext> context = maybeGatherContext(RecordContext::STATE);
  ...
  std::vector<SegmentInfo> snapshot(MempoolId_t mempool_id) {
    ...
    for (const Block* const head_block : all_blocks) {
      ...
      while (block != nullptr && block->mapped) {
        ...
        block_info.requested_size = static_cast<int64_t>(block->requested_size);
    ...
    std::sort(
        result.begin(), result.end(), [](const SegmentInfo& a, const SegmentInfo& b) { return a.address < b.address; });

    record_trace(TraceEntry::SNAPSHOT, 0, static_cast<size_t>(total_active), nullptr, 0, mempool_id, nullptr);
    return result;
  }

  std::vector<TraceEntry>
#if TORCH_VER >= TORCH_2_2_0
  trace(std::function<time_t(approx_time_t)> tsc_to_us)
#else
  trace()
#endif
  {
    ...
    result.insert(
        result.end(), alloc_trace->begin() + static_cast<std::ptrdiff_t>(alloc_trace_next), alloc_trace->end());
    result.insert(
        result.end(), alloc_trace->begin(), alloc_trace->begin() + static_cast<std::ptrdiff_t>(alloc_trace_next));
  ...
  void createOrIncrefPool(MempoolId_t mempool_id, std::shared_ptr<SUPAAllocator> allocator) {
  ...
  // See Note [Interaction with SUPA graph capture]

  // Called by SUPAGraph::capture_begin
  void beginAllocateToPool(MempoolId_t mempool_id, std::function<bool(supaStream_t)> filter) {
    std::lock_guard<std::recursive_mutex> lock(mutex);
    create_or_incref_pool(mempool_id);
  ...
  // Called by SUPAGraph::capture_end
  ...
  // Called by SUPAGraph::reset and MemPool::~MemPool()
  void releasePool(MempoolId_t mempool_id) {
    std::lock_guard<std::recursive_mutex> lock(mutex);
    // The instantiated supaGraphExec_t has been destroyed. We can't blindly
    // delete and supaFree the mempool its capture used, because
    ...
    // supaFree blocks from this graph's pool when it discovers they're unused
    // (unsplit).
    auto* pp = get_private_pool(mempool_id);
    auto uc = --(pp->use_count);
    TORCH_INTERNAL_ASSERT(uc >= 0);
    if (uc == 0) {
      // Allows free_cached_blocks to begin supaFreeing this pool's memory,
  ...
  void addPeerAccess(c10::DeviceIndex dev_to_access) {
    if (std::find(devices_with_peer_access_.begin(), devices_with_peer_access_.end(), dev_to_access) !=
 ...
 private:
  ...
  void create_or_incref_pool(MempoolId_t mempool_id, std::shared_ptr<SUPAAllocator> allocator = nullptr) {
    auto it = graph_pools.find(mempool_id);
    if (it == graph_pools.end()) {
      // mempool_id does not reference an existing pool.
      // Makensur a new pool for CUDAGraph capture or torch.cuda.use_mem_pool
      // usage. use_count is initially 1, which means the pool is
      // being used since somebody called createOrIncrefPool.
      graph_pools.emplace(mempool_id, std::make_unique<PrivatePool>(mempool_id, allocator));
    } else {
  ...
  Block* find_expandable_block(c10::DeviceIndex device, supaStream_t stream, BlockPool* pool, size_t size) {
    ...
    expandable_segments_.emplace_back(new ExpandableSegment(
        device, stream, segment_size, devices_with_peer_access_)); // NOLINT(cppcoreguidelines-owning-memory)

    ExpandableSegment* es = expandable_segments_.back();
    Block* candidate =
        new Block(device, stream, es->size(), pool, es->ptr()); // NOLINT(cppcoreguidelines-owning-memory)
  ...
  bool map_block(Block* to_map, size_t size, const std::shared_ptr<GatheredContext>& ctx) {
    ...
    if (mapped_range.size < to_map->size) {
      ...
      Block* remaining = new Block( // NOLINT(cppcoreguidelines-owning-memory)
          to_map->device,
          to_map->stream,
...
#if TORCH_VER >= TORCH_2_5_0
    for_each_selected_stat_type(
        stat_types, [&](size_t stat_type) { stats.reserved_bytes[stat_type].increase(mapped_range.size); });
#else
    for_each_selected_stat_type(
...
#if TORCH_VER >= TORCH_2_5_0
    auto reserved_bytes_gauge = STATIC_GAUGE(pytorch.CUDACachingAllocator.reserved_bytes);
    reserved_bytes_gauge.record(stats.reserved_bytes[static_cast<int64_t>(StatType::AGGREGATE)].current);
#endif
  ...
  void free_block(Block* block, const std::shared_ptr<GatheredContext>& context) {
    ...
    for_each_selected_stat_type(stat_types, [&](size_t stat_type) {
      ...
      if (!block->expandable_segment_) {
        ...
        if (net_change_inactive_split_blocks > 0) {
          stats.inactive_split[stat_type].increase(static_cast<size_t>(net_change_inactive_split_blocks));
        } else if (net_change_inactive_split_blocks < 0) {
          stats.inactive_split[stat_type].decrease(static_cast<size_t>(-net_change_inactive_split_blocks));
        }
        if (net_change_inactive_split_size > 0) {
          stats.inactive_split_bytes[stat_type].increase(static_cast<size_t>(net_change_inactive_split_size));
        } else if (net_change_inactive_split_size < 0) {
          stats.inactive_split_bytes[stat_type].decrease(static_cast<size_t>(-net_change_inactive_split_size));
        }
#else
   ...
   * or 0 on failure. */
  static size_t try_merge_blocks(Block* dst, Block* src, BlockPool& pool) {
    ...
    delete src; // NOLINT(cppcoreguidelines-owning-memory)

    return subsumed_size;
  }

  BlockPool& get_pool(size_t size, supaStream_t stream) {
    ...
    // supaStreamCaptureStatus (which does a TLS lookup).
    if (C10_UNLIKELY(!captures_underway.empty())) {
  ...
  static bool should_split(const Block* block, size_t size) {
    ...
    if (block->pool->is_small || SUPAAllocatorConfig::expandable_segments()) {
      return remaining >= kMinBlockSize;
    }
    return (size < SUPAAllocatorConfig::max_split_size()) && (remaining > kSmallSize);
  }
  ...
  bool get_free_block(AllocParams& p) const {
    ...
    if ((*it)->expandable_segment_) {
      if (SUPAAllocatorConfig::expandable_segments()) {
    ...
    if ((p.size() < SUPAAllocatorConfig::max_split_size()) && ((*it)->size >= SUPAAllocatorConfig::max_split_size())) {
      return false;
    }
    // Allow oversized block size to be rounded up but within a limit
    if ((p.size() >= SUPAAllocatorConfig::max_split_size()) && ((*it)->size >= p.size() + kLargeBuffer)) {
      return false;
    }
    p.block = *it;
  ...
  static bool trigger_free_memory_callbacks(AllocParams& p) {
    ...
    for (const auto& name : FreeSUPAMemoryCallbacksRegistry()->Keys()) {
      freed_memory |= FreeSUPAMemoryCallbacksRegistry()->Create(name)->Execute();
    }
    return freed_memory;
  }
  ...
  void garbage_collect_cached_blocks(const std::shared_ptr<GatheredContext>& context) {
    ...
    const auto gc_threshold = static_cast<size_t>(
        SUPAAllocatorConfig::garbage_collection_threshold() * static_cast<double>(allowed_memory_maximum));
    ...
    for (const auto& b : large_blocks.blocks) {
      if (!b->is_split()) {
        total_age += static_cast<double>(b->gc_count());
  ...
  // this function. We do supaMalloc sync call in this function which
  // can be expensive while holding the lock. Hence, we pass-in the lock to the
  // function to temporarily release the lock before supaMalloc call and acquire
  ...
  bool alloc_block(
    ...
    // Defensively checks for preexisting SUPA error state.
    ...
    if (set_fraction && total_allocated_memory + size > allowed_memory_maximum) {
      p.err = supaErrorMemoryAllocation;
      return false;
    }
    if (SUPAAllocatorConfig::expandable_segments() &&
      ...
      if (p.block) {
        ...
        if (p.pool->owner_PrivatePool) {
          // The block is for a SUPA graph's PrivatePool.
          p.pool->owner_PrivatePool->supaMalloc_count++;
        }
      } else {
        p.err = supaErrorMemoryAllocation;
      }
    ...
    if (SUPAAllocatorConfig::release_lock_on_supamalloc()) {
      // At scope exit, acquire the lock again. This provides safety against
      // any potential exceptions in the supaMallocMaybeCapturing function.
      auto sg = c10::make_scope_exit([&]() { lock.lock(); });
      lock.unlock();
      p.err = supaMallocMaybeCapturing(&ptr, size, p);
    } else {
      p.err = supaMallocMaybeCapturing(&ptr, size, p);
    }
    if (SUPAAllocatorConfig::release_lock_on_supamalloc()) {
      TORCH_CHECK(lock.owns_lock(), "Failed to acquire lock after supaMalloc");
    }

    if (p.err != supaSuccess) {
      if (p.err == supaErrorMemoryAllocation) {
        // If this is the first attempt (!isRetry), we can forgive and clear
        // SUPA's internal error state.
        //
    ...
    p.block = new Block(
        p.device(), p.stream(), size, p.pool, static_cast<char*>(ptr)); // NOLINT(cppcoreguidelines-owning-memory)
    ...
    if (size >= SUPAAllocatorConfig::max_split_size()) {
#if TORCH_VER >= TORCH_2_5_0
...
#if TORCH_VER >= TORCH_2_5_0
    auto reserved_bytes_gauge = STATIC_GAUGE(pytorch.SUPACachingAllocator.reserved_bytes);
    reserved_bytes_gauge.record(stats.reserved_bytes[static_cast<int64_t>(StatType::AGGREGATE)].current);
#endif

    // p.block came from new, not supaMalloc. It should not be nullptr here.
  ...
  bool release_available_cached_blocks(const AllocParams& p, const std::shared_ptr<GatheredContext>& context) {
    if (SUPAAllocatorConfig::max_split_size() == std::numeric_limits<size_t>::max()) {
      return false;
    }
    BlockPool& pool = *p.pool;
  ...
  bool release_cached_blocks(const std::shared_ptr<GatheredContext>& context, MempoolId_t mempool_id) {
    ...
    for (auto it = graph_pools_freeable.begin(); it != graph_pools_freeable.end();) {
      ...
      if (it->second->supaMalloc_count == 0) {
  ...
  void release_expandable_segment(Block* block) {
    ...
    delete block->expandable_segment_; // NOLINT(cppcoreguidelines-owning-memory)
    delete block; // NOLINT(cppcoreguidelines-owning-memory)
  }
  ...
  void release_block(Block* block, const std::shared_ptr<GatheredContext>& context) {
    ...
    if (active_pool && active_pool->allocator() && pool->owner_PrivatePool) {
      ...
      active_pool->allocator()->raw_delete(static_cast<void*>(block->ptr));
    } else {
      C10_SUPA_CHECK(supaFree(static_cast<void*>(block->ptr)));
    }
    total_allocated_memory -= block->size;

    if (pool->owner_PrivatePool) {
      // The supaFreed block belonged to a SUPA graph's PrivatePool.
      TORCH_INTERNAL_ASSERT(pool->owner_PrivatePool->supaMalloc_count > 0);
      pool->owner_PrivatePool->supaMalloc_count--;
    }
  ...
  void unmap_block(Block* block, const std::shared_ptr<GatheredContext>& context) {
    ...
    if (before_size > 0) {
      ...
      Block* before_free = new Block(
          block->device,
          block->stream,
          static_cast<size_t>(before_size),
          block->pool,
          block->ptr); // NOLINT(cppcoreguidelines-owning-memory)
    ...
    if (after_size > 0) {
      ...
      Block* after_free = new Block( // NOLINT(cppcoreguidelines-owning-memory)
          block->device,
          block->stream,
          after_size,
          block->pool,
...
#if TORCH_VER >= TORCH_2_5_0
    for_each_selected_stat_type(
        stat_types, [&](size_t stat_type) { stats.reserved_bytes[stat_type].decrease(unmapped.size); });
#else
    for_each_selected_stat_type(
    ...
    if (block->pool->owner_PrivatePool) {
      // The supaFreed block belonged to a SUPA graph's PrivatePool.
      TORCH_INTERNAL_ASSERT(block->pool->owner_PrivatePool->supaMalloc_count > 0);
      block->pool->owner_PrivatePool->supaMalloc_count--;
    }
  ...
  static EventPool::Event create_event_internal(c10::DeviceIndex idx) {
    // Leak the event pool to avoid shutdown issues.
    static auto* event_pool = new EventPool(); // NOLINT(cppcoreguidelines-owning-memory)
    return event_pool->get(idx);
  }

  void synchronize_and_free_events(const std::shared_ptr<GatheredContext>& context, PrivatePool* pool = nullptr) {
    ...
    for (auto it = supa_events.begin(); it != supa_events.end();) {
      for (auto e = it->second.begin(); e != it->second.end();) {
        ...
        C10_SUPA_CHECK(supaEventSynchronize(*event));
      ...
      // supa event from the events map
      if (it->second.empty()) {
        it = supa_events.erase(it);
      } else {
        it++;
      }
    }
  ...
  void remove_supagraph_stream_uses(Block* block) {
    // remove stream uses added during supagraph capture
    // (i.e., block->stream_uses - block->supagraph_stream_uses)
    if (C10_UNLIKELY(block_to_supagraph_stream_uses.find(block) != block_to_supagraph_stream_uses.end())) {
      stream_set streams(std::move(block->stream_uses));
      AT_ASSERT(block->stream_uses.empty());
      for (auto& stream : streams) {
        if (block_to_supagraph_stream_uses[block].find(stream) == block_to_supagraph_stream_uses[block].end()) {
          block->stream_uses.insert(stream);
        }
      }
      block_to_supagraph_stream_uses.erase(block);
    }
  }

  void insert_events(Block* block) {
    c10::DeviceIndex prev_device = 0;
    C10_SUPA_CHECK(c10::supa::GetDevice(&prev_device));

    stream_set streams(std::move(block->stream_uses));
    AT_ASSERT(block->stream_uses.empty());
    for (auto& stream : streams) {
      C10_SUPA_CHECK(c10::supa::SetDevice(stream.device_index()));

      EventPool::Event event = create_event_internal(stream.device_index());
      C10_SUPA_CHECK(supaEventRecord(*event, stream.stream()));

      block->event_count++;
      supa_events[stream].emplace_back(std::move(event), block);
    }

    C10_SUPA_CHECK(c10::supa::MaybeSetDevice(prev_device));
  }

  void insert_events_deferred_until_no_capture(const std::shared_ptr<GatheredContext>& context) {
    if (C10_UNLIKELY(!needs_events_deferred_until_no_capture.empty())) {
      for (auto* block : needs_events_deferred_until_no_capture) {
        TORCH_INTERNAL_ASSERT(!block->stream_uses.empty());
        remove_supagraph_stream_uses(block);
        insert_events(block);
        if (block->event_count == 0) {
          free_block(block, context);
        }
  ...
  void process_events(const std::shared_ptr<GatheredContext>& context) {
    ...
    // Process outstanding supaEvents. Events that are completed are
    ...
    for (auto it = supa_events.begin(); it != supa_events.end();) {
      ...
      while (!it->second.empty()) {
        ...
        supaError_t err = C10_SUPA_ERROR_HANDLED(supaEventQuery(*event));
...
// go straight to supaMalloc.  This setting is useful when debugging GPU memory
// errors, since the caching allocator foils supa-memcheck.
static bool forceUncachedAllocator() {
  static bool force_uncached = getenv("PYTORCH_NO_SUPA_MEMORY_CACHING") != nullptr;
...
static void uncached_delete(void* ptr) {
  ...
  if (C10_UNLIKELY(interp)) {
    // in torch 2.3, trace_gpu does not support device
#if TORCH_VER >= TORCH_2_4_0
    (*interp)->trace_gpu_memory_deallocation(c10::kPrivateUse1, reinterpret_cast<uintptr_t>(ptr));
#else
...
class NativeCachingAllocator : public SUPAAllocator {
 public:
  NativeCachingAllocator() = default;
 ...
 public:
  ...
  void malloc(void** devPtr, c10::DeviceIndex device, size_t size, supaStream_t stream) {
    ...
    *devPtr = (void*)block->ptr;
    ...
    if (C10_UNLIKELY(interp)) {
#if TORCH_VER >= TORCH_2_4_0
      (*interp)->trace_gpu_memory_allocation(c10::kPrivateUse1, reinterpret_cast<uintptr_t>(*devPtr));
#else
  ...
  void free(void* ptr) {
    ...
    if (C10_UNLIKELY(interp)) {
#if TORCH_VER >= TORCH_2_4_0
      (*interp)->trace_gpu_memory_deallocation(c10::kPrivateUse1, reinterpret_cast<uintptr_t>(block->ptr));
#else
      (*interp)->trace_gpu_memory_deallocation(reinterpret_cast<uintptr_t>(block->ptr));
#endif
    }
  ...
  double getMemoryFraction(c10::DeviceIndex device) override {
    TORCH_INTERNAL_ASSERT(
        ...
        device,
        ": did you call init?");
    C10_SUPA_CHECK(c10::supa::SetDevice(device));
  ...
  void recordStream(const DataPtr& ptr, supa::SUPAStream stream) override {
    ...
    // This usually happens when SUPA tensors are shared across processes,
  ...
  SnapshotInfo snapshot(MempoolId_t mempool_id) override {
    ...
    auto tsc_to_us = [=](approx_time_t t_approx) { return tsc_to_ns(t_approx) / 1000; };
#endif
    SnapshotInfo result;
    for (auto& da : device_allocator) {
#if TORCH_VER >= TORCH_2_2_0
    ...
    md.garbage_collection_threshold = SUPAAllocatorConfig::garbage_collection_threshold();
    md.max_split_size = SUPAAllocatorConfig::max_split_size();
    md.pinned_num_register_threads = SUPAAllocatorConfig::pinned_num_register_threads();
    md.expandable_segments = SUPAAllocatorConfig::expandable_segments();
    md.release_lock_on_malloc = SUPAAllocatorConfig::release_lock_on_supamalloc();
    md.pinned_use_host_register = SUPAAllocatorConfig::pinned_use_supa_host_register();
  ...
  CheckpointDelta setCheckpointPoolState(c10::DeviceIndex device, std::shared_ptr<AllocatorState> as) override {
    ...
    for (Block* block : rr.allocations_created) {
      ...
      cpd.dataptrs_allocd.emplace_back(
          block->ptr, block->ptr, &local_raw_delete, Device(c10::DeviceType::PrivateUse1, device));
    }
...
#endif
  {
    ...
    TORCH_CHECK_WITH(
        OutOfMemoryError, size < one_exa_bytes, "SUPA out of memory. Tried to allocate more than 1EB memory.");
    c10::DeviceIndex device = 0;
    C10_SUPA_CHECK(c10::supa::GetDevice(&device));
    void* devPtr = nullptr;
    void (*deleteFunc)(void*) = &local_raw_delete;
    SUPAStream stream = supa::getCurrentSUPAStream(device);

    if (forceUncachedAllocator()) {
      deleteFunc = &uncached_delete;

      // Deliberately don't use supaMallocMaybeCapturing here, to force an error
      // if someone tries to use forceUncachedAllocator while capturing.
      C10_SUPA_CHECK(supaMalloc(&devPtr, size));
      const c10::impl::PyInterpreter* interp = c10::impl::GPUTrace::get_trace();
      if (C10_UNLIKELY(interp)) {
#if TORCH_VER >= TORCH_2_4_0
        (*interp)->trace_gpu_memory_allocation(c10::kPrivateUse1, reinterpret_cast<uintptr_t>(devPtr));
...
#else
        const_cast<NativeCachingAllocator*>(this)->malloc(&devPtr, device, size, stream);
#endif
      }
    }
    ...
    return {devPtr, devPtr, deleteFunc, Device(c10::DeviceType::PrivateUse1, device)};
  }
  ...
  void createOrIncrefPool(c10::DeviceIndex device, MempoolId_t mempool_id, std::shared_ptr<SUPAAllocator> allocator)
      override {
    assertValidDevice(device);
    device_allocator[device]->createOrIncrefPool(std::move(mempool_id), std::move(allocator));
  }
  ...
  void recordHistory(bool enabled, CreateContextFn context_recorder, size_t alloc_trace_max_entries, RecordContext when)
      override {
    for (auto& allocator : device_allocator) {
  ...
  void ensureExistsAndIncrefPool(c10::DeviceIndex device, MempoolId_t mempool_id) override {
    assertValidDevice(device);
    device_allocator[device]->ensureExistsAndIncrefPool(std::move(mempool_id));
  }

  // SUPAGraph interactions
  void beginAllocateToPool(c10::DeviceIndex device, MempoolId_t mempool_id, std::function<bool(supaStream_t)> filter)
      override {
    assertValidDevice(device);
  ...
  void* raw_alloc(size_t nbytes) override {
    ...
    C10_SUPA_CHECK(c10::supa::GetDevice(&device));
    void* r = nullptr;
    malloc(&r, device, nbytes, supa::getCurrentSUPAStream(device));
    return r;
  }
  ...
  supaError_t memcpyAsync(
    ...
    if (p2p_enabled || // memcpy ok because memory is mapped in both devices
        ...
        // memcpy ok because both dst and src must have come from supaMalloc
    ...
    // when p2p is not enabled, only supaMemcpyPeerAsync correctly handles
    // memory not allocated via supaMalloc
    return supaMemcpyPeerAsync(dst, dstDevice, src, srcDevice, count, stream);
  }
  ...
  struct MemHandleCacheEntry {
    MemHandleCacheEntry(c10::DeviceIndex device, std::string& handle, const DeviceCachingAllocator& allocator)
        : device_(device) {
      int type = SHAREABLE_SUPA_MALLOC;
      std::istringstream ss(handle);
      if (handle.size() != SUPA_IPC_HANDLE_SIZE) {
        auto version = ss.get();
        TORCH_CHECK(
      ...
      if (type == SHAREABLE_SUPA_MALLOC) {
        supaIpcMemHandle_t supa_handle;
        ss.read((char*)&supa_handle, SUPA_IPC_HANDLE_SIZE);
        C10_SUPA_CHECK(supaIpcOpenMemHandle(&supa_ipc_ptr_, supa_handle, supaIpcMemLazyEnablePeerAccess));
      } else if (type == SHAREABLE_SUPA_EXPANDABLE_SEGMENT) {
        expandable_segment_ = ExpandableSegment::fromShared(device, allocator.peers(), ss).release();
      } else {
    ...
    void clear() {
      if (supa_ipc_ptr_) {
        supa::SUPAGuard device_guard(device_);
        C10_SUPA_CHECK(supaIpcCloseMemHandle(supa_ipc_ptr_));
        supa_ipc_ptr_ = nullptr;
      }
    ...
    void* ptr() const {
      if (supa_ipc_ptr_) {
        return supa_ipc_ptr_;
      }
      return expandable_segment_->ptr();
    }
    c10::DeviceIndex device_;
    ExpandableSegment* expandable_segment_{nullptr};
    void* supa_ipc_ptr_{nullptr}; // nullptr if expandable_segment_ is not null
  ...
  std::shared_ptr<void> getIpcDevPtr(std::string handle) override {
    ...
    C10_SUPA_CHECK(c10::supa::GetDevice(&curr_device));
    auto inserted = ipcMemHandle_to_devptr.insert(
        iter, {handle, MemHandleCacheEntry(curr_device, handle, *device_allocator[curr_device])});
...
struct BackendStaticInitializer {
  BackendStaticInitializer() {
    allocator.store(&Native::caching_allocator);
  }
};

std::atomic<SUPAAllocator*> allocator;
