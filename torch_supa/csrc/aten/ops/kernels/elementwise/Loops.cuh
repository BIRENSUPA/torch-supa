...
__host__ __device__ inline void elementwise_kernel_helper(func_t f, policy_t policy) {
...
#include "torch_supa/csrc/aten/ops/kernels/elementwise/CUDALoops.cuh"

namespace at:: native {

template <typename func_t>
void gpu_kernel_nocast(
    TensorIteratorBase& iter,
    const func_t& f,
    int tuned_elems_per_thread = 0) {

  for (int arg = 0; arg < iter.ntensors(); arg++) {
    TORCH_INTERNAL_ASSERT(
      iter.device(arg).is_privateuseone(),
      "argument ", arg, ": expected a SUPA device but found ", iter.device(arg));
  }
  ...
  if (!iter.can_use_32bit_indexing()) {
    for (auto& sub_iter : iter.with_32bit_indexing()) {
      gpu_kernel_nocast(sub_iter, f, tuned_elems_per_thread);
    }
    return;
  }

  gpu_kernel_impl_nocast(iter, f, tuned_elems_per_thread);
}
...
void gpu_kernel(
  ...
  if (!iter.can_use_32bit_indexing()) {
    for (auto& sub_iter : iter.with_32bit_indexing()) {
      gpu_kernel(sub_iter, f, tuned_elems_per_thread);
    }
    return;
  }

  gpu_kernel_impl(iter, f, tuned_elems_per_thread);
}
...
struct AUnaryFunctor {
  ...
  __host__ __device__ return_t operator()(arg2_t b) const {
    return f(a, b);
  }
...
struct BUnaryFunctor {
  ...
  __host__ __device__ return_t operator()(arg1_t a) const {
    return f(a, b);
  }
  // NB: scalar is stored in higher precision!
...
struct BinaryFunctor {
  __host__ __device__ return_t operator()(arg1_t a, arg2_t b) const {
    return f(a, b);
  }
...
void opmath_gpu_kernel_with_scalars(TensorIteratorBase& iter, const func_t& f) {
  ...
  if (iter.is_cpu_scalar(1)) {
    ...
    const c10::supa::OptionalSUPAGuard device_guard(iter.device(1));
    gpu_kernel(iter, af);
