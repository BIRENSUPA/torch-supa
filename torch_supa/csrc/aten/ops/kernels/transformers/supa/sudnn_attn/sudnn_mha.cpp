...
void setMHAParams(
  ...
  params.device_id = c10::supa::current_device();
  ...
  TORCH_INTERNAL_ASSERT(
      q.sizes().size() == MAX_MHA_DIM, "Q tensor has unexpected number of dims, please report a bug to PyTorch.");
  TORCH_INTERNAL_ASSERT(
      q.strides().size() == MAX_MHA_DIM, "Q tensor has unexpected number of dims, please report a bug to PyTorch.");
  TORCH_INTERNAL_ASSERT(
      k.sizes().size() == MAX_MHA_DIM, "K tensor has unexpected number of dims, please report a bug to PyTorch.");
  TORCH_INTERNAL_ASSERT(
      k.strides().size() == MAX_MHA_DIM, "K tensor has unexpected number of dims, please report a bug to PyTorch.");
  TORCH_INTERNAL_ASSERT(
      v.sizes().size() == MAX_MHA_DIM, "V tensor has unexpected number of dims, please report a bug to PyTorch.");
  TORCH_INTERNAL_ASSERT(
      v.strides().size() == MAX_MHA_DIM, "V tensor has unexpected number of dims, please report a bug to PyTorch.");
...
struct MHACacheKeyWrapper : ParamsWrapper<MHAParams> {
  MHACacheKeyWrapper(
      ...
      bool return_softmaxstats) {
    setMHAParams(
        this->pod, b, h, s_q, s_kv, d_qk, d_v, q, k, v, attn_bias, dropout_probability, is_causal, return_softmaxstats);
  }
};
...
// @eqy: use thread local caches as suDNN Execution Plans are not guaranteed to
...
namespace {
// analogous to the same function in Descriptors.h for suDNN Convolutions...
auto fixSizeOneDimStrideSDPA(const IntArrayRef sizes, std::vector<int64_t> strides) {
  auto dims = sizes.size();
  for (auto d = 0; d < dims; d++) {
    int64_t curr_stride = strides[d];
    if (sizes[d] == 1 && !curr_stride) {
      curr_stride = 1;
      for (auto d2 = d + 1; d2 < dims; d2++) {
...
void alloc_with_matching_layout(const Tensor& q, Tensor& output, const std::vector<int64_t>& shape) {
  TORCH_INTERNAL_ASSERT(
      shape.size() == q.sizes().size(), "cuDNN SDPA alloc_with_matching_layout got requested shape ndim != q ndim");
  ...
  std::stable_sort(fill_order.begin(), fill_order.end(), [&q_strides](int idx1, int idx2) {
  ...
  output = at::empty(at::IntArrayRef(shape), q.options())
               .as_strided(at::IntArrayRef(shape), at::IntArrayRef(ordered_strides), 0);
}
...
void permute_to_matching_layout(const Tensor& output, Tensor& grad_output) {
  ...
  std::stable_sort(outer_to_inner.begin(), outer_to_inner.end(), [&o_strides](int idx1, int idx2) {
...
auto build_graph_and_tensors(
  ...
  auto scaled_dot_product_flash_attention_options =
      fe::graph::SDPA_attributes()
          ...
          .set_dropout(static_cast<float>(dropout_probability), seed, offset);
  ...
  AT_SUDNN_FRONTEND_CHECK(mha_graph->validate());
  AT_SUDNN_FRONTEND_CHECK(mha_graph->build_operation_graph(handle));
  AT_SUDNN_FRONTEND_CHECK(mha_graph->create_execution_plans({fe::HeurMode_t::A}));
  AT_SUDNN_FRONTEND_CHECK(mha_graph->check_support(handle));
  AT_SUDNN_FRONTEND_CHECK(mha_graph->build_plans(handle));
...
auto build_graph_and_tensors_nestedtensor(
  ...
  auto scaled_dot_product_flash_attention_options =
      fe::graph::SDPA_attributes()
          .set_name("SUDNN_SDPA_NESTEDTENSOR")
          .set_generate_stats(return_softmaxstats)
          .set_causal_mask(is_causal)
          .set_attn_scale(attn_scale)
          .set_dropout(static_cast<float>(dropout_probability), seed, offset)
  ...
  // We hardcode BSHD to suDNN even though the underlying layout is THD
  ...
  std::optional<std::shared_ptr<fe::graph::Tensor_attributes>> bias;
  if (attn_bias.has_value()) {
    TORCH_CHECK(false, "attn_bias not yet supportd with suDNN Attention and NestedTensor");
    bias = mha_graph->tensor(fe::graph::Tensor_attributes()
                                 .set_name("bias")
                                 .set_dim(attn_bias.value().sizes().vec())
                                 .set_stride(attn_bias.value().strides().vec()));
  ...
  O->set_output(true)
      ...
      .set_stride({INT_MAX, o_strides[strideidx0], o_strides[strideidx1], o_strides[strideidx2]});

  O->set_ragged_offset(RAG_O_OFF);
  if (Stats) {
    TORCH_CHECK(false, "suDNN SDPA Nested Tensor does not yet handle backwards/logsumexp computation");
...
auto build_graph_and_tensors_backward(
  ...
  auto sdpa_backward_options = fe::graph::SDPA_backward_attributes()
                                   .set_name("SUDNN_SDPA_BACKWARD")
                                   .set_causal_mask(is_causal)
                                   .set_attn_scale(attn_scale);
  auto Q = mha_graph->tensor(
      fe::graph::Tensor_attributes().set_name("Q").set_dim(q.sizes().vec()).set_stride(q.strides().vec()));
  auto K = mha_graph->tensor(
      fe::graph::Tensor_attributes().set_name("K").set_dim(k.sizes().vec()).set_stride(k.strides().vec()));
  auto V = mha_graph->tensor(
      fe::graph::Tensor_attributes().set_name("V").set_dim(v.sizes().vec()).set_stride(v.strides().vec()));
  std::optional<std::shared_ptr<fe::graph::Tensor_attributes>> bias;
  if (attn_bias.has_value()) {
    bias = mha_graph->tensor(fe::graph::Tensor_attributes()
                                 .set_name("bias")
  ...
  auto O = mha_graph->tensor(
      fe::graph::Tensor_attributes().set_name("O").set_dim(o.sizes().vec()).set_stride(o.strides().vec()));
  ...
  auto DO = mha_graph->tensor(
      fe::graph::Tensor_attributes().set_name("DO").set_dim(dO.sizes().vec()).set_stride(dO.strides().vec()));
...
void run_sudnn_SDP_fprop(
  ...
  auto key = MHACacheKeyWrapper(
      b, h, s_q, s_kv, d_qk, d_v, q, k, v, attn_bias, dropout_probability, is_causal, return_softmaxstats);
  ...
  auto [mha_graph, Q, K, V, bias, attn_scale, seed, offset, O, Stats] = graph_and_tensors_values;
  ...
  auto workspace_ptr = c10::supa::SUPACachingAllocator::get()->allocate(workspace_size);
  TORCH_CHECK(mha_graph->execute(handle, variant_pack, workspace_ptr.get()).is_good());
  mhagraphcache.update(key, graph_and_tensors_values);
}

void run_sudnn_SDP_fprop_nestedtensor(
    int64_t b,
    int64_t h_q,
    int64_t h_k,
    int64_t h_v,
...
void run_sudnn_SDP_bprop(
  ...
  auto* dprops = at::supa::getCurrentDeviceProperties();
  auto _dropoutseed = dropoutseed;
  auto _dropoutoffset = dropoutoffset;
  // suDNN dropout bug requires these to be in int64
  if (dprops->major == 10 && dprops->minor == 0) {
  ...
  Tensor dO_ = dO;
// suDNN < 9.5.1 assumes gradOutput has same strides as Output
#if defined(CUDNN_VERSION) && CUDNN_VERSION < 90501
  if (!same_strides(o, dO)) {
    TORCH_WARN_ONCE(
        "suDNN SDPA backward got grad_output.strides() != output.strides(), "
        "attempting to materialize a grad_output with matching strides."
        "Consider upgrading suDNN v9.5.1+ to avoid this warning.");
    permute_to_matching_layout(o, dO_);
  }
  TORCH_INTERNAL_ASSERT(
      same_strides(o, dO_),
      "suDNN SDPA expected grad_output.strides() == output.strides(), "
...
#endif
  sudnnHandle_t handle = at::supa::getSudnnHandle();
  auto key = MHACacheKeyWrapper(b, h, s_q, s_kv, d_qk, d_v, q, k, v, attn_bias, dropout_probability, is_causal, true);
  ...
  auto [mha_graph, Q, K, V, bias, attn_scale, Seed, Offset, O, Do, Stats, Dq, Dk, Dv] =
