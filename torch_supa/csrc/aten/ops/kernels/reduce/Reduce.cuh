...
struct ReduceConfig {
  ...
  void set_block_dimension(int64_t dim0, int64_t dim1) {
    ...
    block_width = std::min(dim0_pow2, int(at::supa::warp_size()));
...
template <typename scalar_t, typename ops_t, typename index_t, typename out_scalar_t=scalar_t, int vt0=8, int input_vec_size=vt0>
struct ReduceOp {
  ...
  template <int output_vec_size>
  C10_DEVICE __forceinline__ void run() const {
  ...
  template <int output_vec_size>
  C10_DEVICE __forceinline__ std::array<arg_t, output_vec_size> thread_reduce(const scalar_t* data) const {
  ...
  C10_DEVICE __forceinline__ arg_t input_vectorized_thread_reduce_impl(const scalar_t* data) const {
...
inline void launch_jitted_reduce_kernel(
    ...
    int vt0, const ReduceConfig& config, 
 ...
 public:
  ...
  AccumulationBuffer(size_t acc_t_size, size_t out_t_size, char* out_ptr, int64_t size) {
    ...
    if (out_t_size >= acc_t_size) {
      ...
      auto& allocator = *c10::supa::SUPACachingAllocator::get();
...
ReduceConfig setReduceConfig(const TensorIterator& iter){
  ...
  const int num_mp =
      at::supa::getCurrentDeviceProperties()->multiProcessorCount;
#ifdef USE_ROCM
  ...
  int max_threads_per_mp =
      at::supa::getCurrentDeviceProperties()->maxThreadsPerMultiProcessor;
#ifdef USE_ROCM
...
template <typename scalar_t, typename out_scalar_t, int vt0=8, int input_vec_size=vt0, typename ops_t, typename ident_t=double>
inline void gpu_reduce_kernel(TensorIterator& iter, const ops_t& ops, ident_t ident=0,
  ...
  if (config.should_global_reduce()) {
    ...
    auto stream = c10::supa::getCurrentSUPAStream();
    C10_SUPA_CHECK(supaMemsetAsync(semaphores.get(), 0, config.semaphore_size(), stream));
  }
...
template <char const* name, typename scalar_t, typename out_scalar_t, int vt0=8, typename ident_t=double>
inline void jitted_gpu_reduce_kernel(TensorIterator& iter, const std::string& func, ident_t ident=0,
  ...
  static std::vector<std::array<at::cuda::jit::NvrtcFunction, 3>> fn_cache(c10::supa::device_count());
