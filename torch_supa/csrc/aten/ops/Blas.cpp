...
c10::MaybeOwned<Tensor> inline prepare_matrix_for_sublas(
    const Tensor& tensor,
  ...
  if ((tensor_strides[1] == 1) && (tensor_strides[0] >= std::max<int64_t>(1, tensor_sizes[1]))) {
...
c10::MaybeOwned<Tensor> inline prepare_matrix_for_sublas(const Tensor& tensor, bool& transpose_tensor) {
  if (tensor.is_non_overlapping_and_dense()) { // common case
    transpose_tensor = tensor.is_contiguous();
...
struct sublasCommonArgs {
  sublasCommonArgs(const Tensor& mat1, const Tensor& mat2, Tensor& c) {
    bool transpose_result = false;
    bool transpose_mat1 = false;
    bool transpose_mat2 = false;
    result = prepare_matrix_for_sublas(c, transpose_result);
    mata = prepare_matrix_for_sublas(transpose_result ? mat2 : mat1, transpose_mat1, transpose_result);
    matb = prepare_matrix_for_sublas(transpose_result ? mat1 : mat2, transpose_mat2, transpose_result);
...
c10::MaybeOwned<Tensor> prepare_batch_matrix_for_sublas(
    const Tensor& tensor,
    bool& transpose_tensor,
    int64_t& ld_tensor,
    bool transpose_result,
...
sublas::GEMMAndBiasActivationEpilogue activation_to_gemm_and_blas_arg(Activation a) {
  switch (a) {
    case Activation::None:
      return sublas::GEMMAndBiasActivationEpilogue::None;
    case Activation::RELU:
      return sublas::GEMMAndBiasActivationEpilogue::RELU;
    case Activation::GELU:
      return sublas::GEMMAndBiasActivationEpilogue::GELU;
    default:
      TORCH_CHECK(false);
      return sublas::GEMMAndBiasActivationEpilogue::None;
  }
...
bool getDisableAddmmSublasLt() {
  static const auto env_value = torch_supa::utils::get_env("DISABLE_ADDMM_CUDA_LT");
  return env_value == "1";
}
...
Tensor& addmm_out_impl(
  ...
  // Make sure to keep addmm below in sync with this code; it
  ...
  static bool disable_addmm_sublas_lt = getDisableAddmmSublasLt();
  at::ScalarType scalar_type = self.scalar_type();
  c10::MaybeOwned<Tensor> self_;
  if (&result != &self) {
    if (!disable_addmm_sublas_lt) {
      useLtInterface = beta.toComplexDouble() == 1.0 && self.dim() == 1 && result.dim() == 2 &&
          self.sizes()[0] == mat2_sizes[1] && self.is_contiguous() && result.is_contiguous() &&
          (scalar_type == at::ScalarType::Double || scalar_type == at::ScalarType::Float ||
           scalar_type == at::ScalarType::Half || scalar_type == at::ScalarType::BFloat16) &&
          mat2_sizes[0] > 1 && mat2_sizes[1] > 1 && mat2_sizes[0] < 65535 * 32 && mat2_sizes[1] < 65535 * 32 &&
  ...
  sublasCommonArgs args(mat1, mat2, result);
  ...
  if (useLtInterface) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, scalar_type, "addmm_supa_lt", [&] {
      at::sublas::gemm_and_bias<scalar_t>(
          args.transa == 't',
          args.transb == 't',
          args.m,
          args.n,
    ...
    AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
        at::ScalarType::Half, at::ScalarType::BFloat16, scalar_type, "addmm_supa", [&] {
...
const Tensor& baddbmm_out_impl(
  ...
  if ((result_strides[1] == 1) &&
    ...
    result_ =
        c10::MaybeOwned<Tensor>::owned(result.transpose(1, 2).clone(at::MemoryFormat::Contiguous).transpose(1, 2));
  }
  ...
  int64_t ldb = 0;
  int64_t ldc = 0;
  bool transpose_batch1 = false;
  bool transpose_batch2 = false;
  auto batch1_ = prepare_batch_matrix_for_sublas(
      transpose_result ? batch2 : batch1, transpose_batch1, lda, transpose_result, m, k);
  auto batch2_ = prepare_batch_matrix_for_sublas(
      transpose_result ? batch1 : batch2, transpose_batch2, ldb, transpose_result, k, n);
  ...
  AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      at::ScalarType::Half, at::ScalarType::BFloat16, self.scalar_type(), "baddbmm_supa", [&] {
        using opmath_t = at::opmath_type<scalar_t>;
        opmath_t alpha_val = alpha.to<opmath_t>();
        opmath_t beta_val = beta.to<opmath_t>();
        ...
        if (num_batches == 1) {
          at::sublas::gemm<scalar_t>(
              transa, transb, m, n, k, alpha_val, batch1_ptr, lda, batch2_ptr, ldb, beta_val, result_ptr, ldc);
        } else {
          at::sublas::bgemm<scalar_t>(
              transa,
              transb,
              m,
              n,
...
(const Tensor& self,
 ...
 const Tensor& result) {
  ...
  addmm_out_impl(const_cast<Tensor&>(result), self, mat1, mat2, beta, alpha);
}
...
SUPA_IMPL_FUNC(mm)(const Tensor& self, const Tensor& mat2, const Tensor& result) {
  addmm_out_impl(const_cast<Tensor&>(result), result, self, mat2, 0, 1);
}

SUPA_IMPL_FUNC(baddbmm)
(const Tensor& self,
 const Tensor& batch1,
 const Tensor& batch2,
 const Scalar& beta,
 ...
 const Tensor& result) {
  {
    ...
    baddbmm_out_impl(result, self, batch1, batch2, beta, alpha);
  }
}

SUPA_IMPL_FUNC(bmm)(const Tensor& batch1, const Tensor& batch2, const Tensor& result) {
  Scalar beta(0.0);
  Scalar alpha(1.0);
  {
    NoNamesGuard guard;
    baddbmm_out_impl(result, result, batch1, batch2, beta, alpha);
  }
}
...
inline void dot_check(const Tensor& self, const Tensor& other) {
  ...
  TORCH_CHECK(
      (self.numel() <= INT_MAX) && (self.stride(0) <= INT_MAX) && (other.stride(0) <= INT_MAX),
...
Tensor& SUPANativeFunctions::dot_out(const Tensor& self, const Tensor& other, Tensor& result) {
  if (self.is_complex()) {
    if (self.is_conj()) {
      if (other.is_conj()) {
        dot_out(self.conj(), other.conj(), result);
        result = result.conj();
        return result;
      }
      return vdot_out(self.conj(), other, result);
    }
    if (other.is_conj()) {
      return vdot_out(other.conj(), self, result);
    }
  }
  ...
  result = AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
      ScalarType::Half, ScalarType::BFloat16, self.scalar_type(), "dot", [&] {
        auto handle = at::supa::getCurrentSuBlasHandle();
        at::sublas::PointerModeGuard pointerModeGuard(handle, SUBLAS_POINTER_MODE_DEVICE);
        at::sublas::dot<scalar_t>(
            handle,
            n,
            self.const_data_ptr<scalar_t>(),
            incx,
...
Tensor& SUPANativeFunctions::vdot_out(const Tensor& self, const Tensor& other, Tensor& result) {
  if (!self.is_complex()) {
    return dot_out(self, other, result);
  }

  if (self.is_conj()) {
    if (other.is_conj()) {
      return vdot_out(other.conj(), self.conj(), result);
    }
    return dot_out(self.conj(), other, result);
  }
  if (other.is_conj()) {
    dot_out(self, other.conj(), result);
    result = result.conj();
    return result;
  }
  ...
  result = AT_DISPATCH_COMPLEX_TYPES(self.scalar_type(), "vdot", [&] {
    auto handle = at::supa::getCurrentSuBlasHandle();
    at::sublas::PointerModeGuard pointerModeGuard(handle, SUBLAS_POINTER_MODE_DEVICE);
    at::sublas::vdot<scalar_t>(
        handle,
        n,
        self.const_data_ptr<scalar_t>(),
        incx,
 ...
 const Tensor& result) {
  ...
  if (mat.numel() == 0) {
    ...
    if (result.numel() != 0) {
      ...
      AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES_AND2(
          at::ScalarType::Half, at::ScalarType::BFloat16, mat.scalar_type(), "addmv_impl", [&] {
            auto beta = beta_.to<scalar_t>();
            auto alpha = alpha_.to<scalar_t>();
            if (mat.stride(0) == 1 && mat.stride(1) >= std::max<int64_t>(1, mat.size(0))) {
              at::sublas::gemv<scalar_t>(
                  'n',
                  mat.size(0),
                  mat.size(1),
                  alpha,
                  mat.const_data_ptr<scalar_t>(),
                  mat.stride(1),
                  vec_contiguous.const_data_ptr<scalar_t>(),
                  vec_stride,
                  beta,
                  result.mutable_data_ptr<scalar_t>(),
                  r_stride);
