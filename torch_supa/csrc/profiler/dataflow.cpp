...
struct RawTensors {
  ...
  void operator()(TensorMetadata& t) {
    tensors_.emplace_back(RawTensorInfo{t.impl(), t.data_, t.device_, false, t.allocation_id_, t.id_});
  }
  ...
  void operator()(ExtraFields<EventType::Allocation>& a) {
    ...
    tensors_.emplace_back(RawTensorInfo{NoTensorImpl, ptr, a.device(), is_free, a.allocation_id_, a.id_});
  }
...
void calculateUniqueTensorIDs(std::vector<std::shared_ptr<Result>>& sorted_results) {
    ...
    for (auto& result : sorted_results) {
      result->visit(c10::overloaded(
          ...
          [&](ExtraFields<EventType::PyCall>& py_call) {
            // torch.nn.Module
            if (py_call.module_.has_value() && seen_modules.insert(py_call.module_->self_).second) {
            ...
            if (py_call.optimizer_.has_value() && seen_optimizers.insert(py_call.optimizer_->self_).second) {
  ...
  ska::flat_hash_set<storage_id_pair_t, HashCombine> same_group_set;
  {
    ...
    for (const auto& t : tensors) {
      ...
      it->second < allocation_id ? same_group_set.insert({it->second, allocation_id})
