...
constexpr size_t CallTypeSize = 4;
...
struct CodeLocation {
  ...
  explicit CodeLocation(PyFrameObject* frame) : line_number_{PyFrame_GetLineNumber(frame)} {
...
PyCodeObject* getCode<CallType::PyModuleCall>() {
  static auto* module_call_code = []() {
    ...
    auto* res = py::module::import("torch.nn").attr("Module").attr("__call__").attr("__code__").ptr();
...
PyCodeObject* getCode<CallType::PyOptimizerCall>() {
  static auto* optimizer_step_code = []() {
    pybind11::gil_scoped_acquire gil;
    auto* res = py::module::import("torch.optim").attr("Optimizer").attr("_optimizer_step_code").attr("__code__").ptr();
    TORCH_INTERNAL_ASSERT(PyCode_Check(res));
    return (PyCodeObject*)res;
  }();
...
template <>
struct std::hash<torch_supa::profiler::impl::CodeLocation> {
  size_t operator()(const torch_supa::profiler::impl::CodeLocation& x) {
...
class CallTypeHelper final {
 private:
  static_assert(CallType::PyCall == 0, "CallTypeHelper uses integer math which depends on a zero start.");
  static constexpr size_t End = CallTypeSize;

  template <size_t... I>
  static constexpr std::tuple<ClassT<(CallType)I>...> make_tuple_impl(std::index_sequence<I...>);
...
template <>
struct Config<CallType::PyModuleCall> : ExtendedPyCallConfig<PyModuleSelf, PyModuleCls, NNModuleInfo::ParameterInfo> {};

template <>
struct Config<CallType::PyOptimizerCall>
    : ExtendedPyCallConfig<PyOptimizerSelf, PyOptimizerCls, OptimizerInfo::ParameterInfo> {};
...
class Callsite {
 public:
  ...
  static_assert(std::is_trivially_copyable_v<key_t>, "Key should be trivial, as it is passed by value.");
...
class ValueCache {
 public:
  ...
  static std::optional<TensorMetadata> recordIfTensor(py::handle p);
  static std::vector<std::pair<std::string, TensorMetadata>> unpackTensorMap(const py::dict& tensor_map);
  void trimPrefixes();

 private:
  template <CallType C>
  typename ExtraFields<Config<C>::event_type>::args_t load(const typename Config<C>::key_t&) const;
...
typename Config<C>::cls_t set_class(
  ...
  if (cache.cls_names_.find(cls) == cache.cls_names_.end()) {
    cache.cls_names_[cls] = at::StringView(py::str(cls_handle.attr("__name__")));
  }
  return cls;
}
...
TensorMetadata toTensorMetadata(PyObject* self) {
  ...
  return TensorMetadata{m, t.sizes().vec(), m.layout_ == at::kStrided ? t.strides().vec() : std::vector<int64_t>()};
}

std::optional<TensorMetadata> ValueCache::recordIfTensor(py::handle p) {
  return THPVariable_CheckExact(p.ptr()) ? std::optional<TensorMetadata>{toTensorMetadata(p.ptr())} : std::nullopt;
}

std::vector<std::pair<std::string, TensorMetadata>> ValueCache::unpackTensorMap(const py::dict& tensor_map) {
...
void ValueCache::store<CallType::PyCall>(const PyCallKey& key, no_ephemeral_t /*unused*/) {
  ...
  if (C10_UNLIKELY(locations.find(key) == locations.end())) {
    locations[key] = {key.line_number_, at::StringView(key.filename_), at::StringView(key.name_)};
  }
}

template <>
ExtraFields<EventType::PyCall>::args_t ValueCache::load<CallType::PyCall>(const PyCallKey& key) const {
...
void ValueCache::store<CallType::PyModuleCall>(
  ...
  if (C10_UNLIKELY(cache.cls_and_parameters_.find(key) == cache.cls_and_parameters_.end())) {
...
template <>
ExtraFields<EventType::PyCall>::args_t ValueCache::load<CallType::PyModuleCall>(const PyModuleCallKey& key) const {
  ...
  NNModuleInfo info{key, cls, cache.cls_names_.at(cls), cls_and_parameters.parameters_};
  return {
...
template <>
ExtraFields<EventType::PyCall>::args_t ValueCache::load<CallType::PyOptimizerCall>(
  ...
  OptimizerInfo info{key, cls, cache.cls_names_.at(cls), cls_and_parameters.parameters_};
  return {
...
template <>
void ValueCache::store<CallType::PyCCall>(const PyCCallKey& key, Config<CallType::PyCCall>::ephemeral_t arg) {
...
template <>
ExtraFields<EventType::PyCCall>::args_t ValueCache::load<CallType::PyCCall>(const PyCCallKey& key) const {
...
void ValueCache::trimPrefixes() {
  static const auto prefixes = []() {
    pybind11::gil_scoped_acquire gil;
    return py::module::import("torch.profiler.python_tracer").attr("_prefix_regex")().cast<std::vector<std::string>>();
  }();
...
struct TraceKeyCacheState {
  ...
  TraceKey intern(Callsite<C> callsite, typename Config<C>::ephemeral_t ephemeral, ValueCache& value_cache) {
  ...
  auto lookup(Callsite<C>& callsite, ValueCache& value_cache) const {
    return std::make_pair(value_cache.load<C>(callsite.value_), value_cache.load<CallType::PyCall>(callsite.caller_));
  }
...
class gil_and_restore_thread {
 public:
  ...
  gil_and_restore_thread(const gil_and_restore_thread&) = delete;
  gil_and_restore_thread& operator=(const gil_and_restore_thread&) = delete;
  gil_and_restore_thread(gil_and_restore_thread&&) = delete;
  gil_and_restore_thread& operator=(gil_and_restore_thread&&) = delete;
...
class PythonTracer;
struct ThreadLocalResults {
  ThreadLocalResults(PyThreadState* thread_state, ValueCache* value_cache, PythonTracer* active_tracer)
  ...
  TraceKey intern(Ephemeral ephemeral, Args... args) {
    static_assert(Config<C>::event_type == E, "ThreadLocalResults.intern called from the wrong typed context.");
...
// ============================================================================
class PythonTracer final : public torch::profiler::impl::python_tracer::PythonTracerBase {
 public:
  PythonTracer(torch::profiler::impl::RecordQueue* queue);
  PythonTracer(const PythonTracer&) = delete;
  PythonTracer& operator=(const PythonTracer&) = delete;
  PythonTracer(PythonTracer&&) = delete;
  PythonTracer& operator=(PythonTracer&&) = delete;
  ...
  std::vector<std::shared_ptr<Result>> getEvents(
      ...
      std::vector<torch::profiler::impl::python_tracer::CompressedEvent>& enters,
 ...
 private:
  void recordPyCall(ThreadLocalResults& tls, PyFrameObject* frame, bool is_startup_frame);
...
std::vector<PyThreadState*> PythonTracer::interpreterThreads() const {
  pybind11::gil_scoped_acquire gil;
...
// safe
PyObject* py_gc_callback = nullptr;
...
PyObject* PythonTracer::gc_event_callback(PyObject* self, PyObject* args) {
  ...
  PythonTracer* instance = reinterpret_cast<PythonTracer*>(PyCapsule_GetPointer(self, nullptr));
  if (!instance) {
    PyErr_SetString(PyExc_RuntimeError, "Invalid tracer instance");
    return nullptr;
  }
  instance->queue_->getSubqueue()->emplace_gc_call(phase, c10::getApproximateTime());
  Py_RETURN_NONE;
}
#endif
...
void PythonTracer::register_gc_callback() {
  ...
  static PyMethodDef method_def = {"gc_event_callback", (PyCFunction)gc_event_callback, METH_VARARGS, nullptr};
...
void PythonTracer::recordPyCall(ThreadLocalResults& tls, PyFrameObject* frame, bool is_startup_frame) {
  ...
  const auto key = [&]() -> TraceKey {
    ...
    if (code.get() == optimizer_hook_) {
      ...
      auto back = THPFrameObjectPtr(PyFrame_GetBack(frame));
      TORCH_INTERNAL_ASSERT(back != nullptr);
      return tls.intern<CallType::PyOptimizerCall, E>(frame, self.get(), back.get());
    }
    auto back = THPFrameObjectPtr(PyFrame_GetBack(frame));
  ...
  is_startup_frame ? start_frames_.push_back({key, time}) : queue_->getSubqueue()->emplace_py_call(key, time);
}

void PythonTracer::recordCCall(ThreadLocalResults& tls, PyFrameObject* frame, PyObject* arg) {
 ...
 public:
  PostProcess(
    ...
    for (size_t python_tid : c10::irange(tls.size())) {
      CallTypeHelper<TraceKeyCacheState>::map(tls[python_tid].trace_keys_, *this, value_cache, python_tid);
  ...
  template <CallType C>
  void operator()(const TraceKeyCacheState<C>& trace_cache, const ValueCache& value_cache, size_t python_tid) {
    for (const auto& it : trace_cache.state_) {
      const auto inserted =
          get_state<Config<C>::event_type>().fields_.insert({it.second, value_cache.load(it.first, python_tid)});
      TORCH_INTERNAL_ASSERT(inserted.second, "Duplicate key: ", it.second);
    }
  }

  template <EventType E, size_t N>
  void addExits(AppendOnlyList<c10::approx_time_t, N>& exits, size_t python_tid) {
    for (const auto i : exits) {
      get_state<E>().exits_.push({time_converter_(i), python_tid});
    }
  }

  std::vector<std::shared_ptr<Result>> run(std::vector<torch::profiler::impl::python_tracer::CompressedEvent>& enters) {
    std::stable_sort(enters.begin(), enters.end(), [](const auto a, const auto b) { return a.enter_t_ < b.enter_t_; });
  ...
  void populate(
    ...
    for (const auto& enter : enters) {
      ...
      if (fields_it != state.fields_.end()) {
        while (!state.exits_.empty() && state.exits_.top().t_ < enter.enter_t_) {
          auto& exit = state.exits_.top();
          pop(stacks[exit.python_tid_], exit.t_);
          state.exits_.pop();
        }
        out.push_back(Result::create(enter.enter_t_, enter.system_tid_, enter.kineto_info_, fields_it->second));
    ...
    ska::flat_hash_map<size_t, std::pair<size_t, torch::profiler::impl::kineto::DeviceAndResource>> tid_map;
    auto it = out.rbegin();
    for ([[maybe_unused]] auto _ : c10::irange(initial_size, out.size())) {
      ...
      if ((*it)->start_tid_ == NoTID && SOFT_ASSERT(E == EventType::PyCall)) {
        const auto& tid_info =
            tid_map.insert({python_tid, {NoTID, torch::profiler::impl::kineto::DeviceAndResource()}}).first->second;
...
std::vector<std::shared_ptr<Result>> PythonTracer::getEvents(
  ...
  PostProcess post_process(std::move(time_converter), thread_local_results_, value_cache_, end_time_ns);
...
class PythonMemoryTracer final : public torch::profiler::impl::python_tracer::PythonMemoryTracerBase {
 public:
  explicit PythonMemoryTracer() = default;
  PythonMemoryTracer(const PythonMemoryTracer&) = delete;
  PythonMemoryTracer& operator=(const PythonMemoryTracer&) = delete;
  PythonMemoryTracer(PythonMemoryTracer&&) = delete;
  PythonMemoryTracer& operator=(PythonMemoryTracer&&) = delete;
...
void toggle_memory_tracing(bool enable) {
  pybind11::gil_scoped_acquire gil;
  THPObjectPtr torch_supa_memory_module(PyImport_ImportModule("torch.supa.memory"));
  if (!torch_supa_memory_module) {
    return;
  }
  THPObjectPtr snapshot_func(PyObject_GetAttrString(torch_supa_memory_module.get(), "_record_memory_history_impl"));
  if (!snapshot_func) {
    return;
  }
...
void PythonMemoryTracer::export_memory_history(const std::string& path) {
  ...
  THPObjectPtr snapshot_func(PyObject_GetAttrString(torch_supa_memory_module.get(), "_dump_snapshot"));
  if (!snapshot_func) {
    return;
  }
...
int PythonTracer::pyProfileFn(PyObject* obj, PyFrameObject* frame, int what, PyObject* arg) {
  auto& local_results = *reinterpret_cast<TraceContext*>(obj)->thread_local_results_;
  switch (what) {
    case PyTrace_CALL:
...
std::unique_ptr<torch::profiler::impl::python_tracer::PythonTracerBase> getTracer(
    torch::profiler::impl::RecordQueue* queue) {
  return std::make_unique<PythonTracer>(queue);
}

#if TORCH_VER >= TORCH_2_8_0
std::unique_ptr<torch::profiler::impl::python_tracer::PythonMemoryTracerBase> getMemoryTracer() {
...
namespace torch_supa::profiler::python_tracer {

void init() {
  pybind11::gil_scoped_acquire gil;
  TORCH_CHECK(PyType_Ready(&torch_supa::profiler::impl::TraceContextType) == 0);
  torch::profiler::impl::python_tracer::registerTracer(&torch_supa::profiler::impl::getTracer);
#if TORCH_VER >= TORCH_2_8_0
  torch::profiler::impl::python_tracer::registerMemoryTracer(&torch_supa::profiler::impl::getMemoryTracer);
#endif
}
} // namespace torch_supa::profiler::python_tracer
