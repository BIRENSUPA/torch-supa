...
auto parseArgData(const std::vector<op_input_t>& input_shapes, const std::vector<op_input_t>& concreteInputs) {
  ...
  if (input_shapes.size() == concreteInputs.size() && !concreteInputs.empty()) {
    ...
    for (const auto& i : c10::irange(input_shapes.size())) {
      std::visit(
          c10::overloaded([&](const c10::IValue& val) { concrete_inputs_list[i] = val; }, [&](const auto&) {}),
          input_shapes[i]);
      std::visit(
          c10::overloaded(
...
struct MetadataBase {
  /* implicit */ MetadataBase(const std::shared_ptr<Result>& result) : kinetoActivity_{result->kineto_activity_} {
  ...
  void addMetadata(const std::string& key, const std::string& value) {
    if (kinetoActivity_ && !value.empty() && value != "\"\"") {
      torch_supa::profiler::impl::kineto::addMetadata(
...
struct AddTensorboardFields : public MetadataBase {
  AddTensorboardFields(const std::shared_ptr<Result>& result, KinetoEvent& kineto_event) : MetadataBase(result) {
    result->visit(*this);
    ...
    result->visit_if_base<PyExtraFieldsBase>([&, this](const auto& i) -> void {
      ...
      while (parent && !parent_id.has_value()) {
        parent->visit_if_base<PyExtraFieldsBase>([&](const auto& j) { parent_id = std::to_string(j.id_); });
...
struct AddGenericMetadata : public MetadataBase {
  AddGenericMetadata(std::shared_ptr<Result>& result, const torch::profiler::impl::ProfilerConfig* config)
      : MetadataBase(result), config_(config) {
    result->visit(*this);
    if (config->experimental_config.verbose) {
      result->visit_if_base<PyExtraFieldsBase>(
          [&, this](const auto& i) -> void { this->addMetadata("Python thread", std::to_string(i.python_tid_)); });
    }
  }
    ...
    for (const auto& [key, val] : op_event.kwinputs_) {
      ...
      if (!val.isInt() && !val.isDouble() && !val.isString() && !val.isBool()) {
        LOG(WARNING) << "Inputted kwarg: " << key
                     << " is not an int, double, string, or bool for op: " << op_event.name_ << " skipping";
        continue;
      }
...
struct KinetoThreadLocalState : public ProfilerStateBase {
  explicit KinetoThreadLocalState(
      const torch_supa::profiler::impl::SupaProfilerConfig& config,
      std::set<torch_supa::profiler::impl::SupaActivityType> activities)
      : ProfilerStateBase(config.base_config), startTime(getTimeNs()), recordQueue(config, std::move(activities)) {}

  ~KinetoThreadLocalState() override = default;
  KinetoThreadLocalState(const KinetoThreadLocalState&) = delete;
  KinetoThreadLocalState& operator=(const KinetoThreadLocalState&) = delete;
  KinetoThreadLocalState(KinetoThreadLocalState&&) = delete;
  KinetoThreadLocalState& operator=(KinetoThreadLocalState&&) = delete;

  static KinetoThreadLocalState* get(bool global) {
    auto* state = ProfilerStateBase::get(/*global=*/global);
    TORCH_INTERNAL_ASSERT_DEBUG_ONLY(state == nullptr || state->profilerType() == ActiveProfilerType::KINETO);
  ...
  void reportVulkanEventToProfiler(torch::profiler::impl::vulkan_id_t id) {
    if (!config_.disabled()) {
      recordQueue.getSubqueue()->emplace_vulkan_event(c10::getApproximateTime(), id);
    }
  }
  ...
  void reportMemoryUsage(
    ...
    if (config_.profile_memory && !config_.disabled()) {
      recordQueue.getSubqueue()->emplace_allocation_event(
          c10::getApproximateTime(), ptr, alloc_size, total_allocated, total_reserved, device.type(), device.index());
    }
  }

  void reportOutOfMemory(int64_t alloc_size, size_t total_allocated, size_t total_reserved, c10::Device device)
      override {
    if (config_.profile_memory && !config_.disabled()) {
      recordQueue.getSubqueue()->emplace_ooms_event(
          c10::getApproximateTime(), alloc_size, total_allocated, total_reserved, device.type(), device.index());
    }
  }
  ...
  std::unique_ptr<torch::profiler::impl::kineto::ActivityTraceWrapper> finalizeTrace() {
    ...
    kinetoEvents.erase(
        std::remove_if(kinetoEvents.begin(), kinetoEvents.end(), [](const auto& i) { return i.isPythonFunction(); }),
        kinetoEvents.end());
  ...
  torch_supa::profiler::impl::SupaRecordQueue recordQueue;
...
void onFunctionExit(const at::RecordFunction& fn, at::ObserverContext* ctx_ptr) {
  ...
  auto* kineto_ctx_ptr = dynamic_cast<torch::profiler::impl::KinetoObserverContext*>(ctx_ptr);
  ...
  if (fn.isNcclMeta()) {
    ...
    torch_supa::profiler::impl::SaveBcclMetaConfig bcclMetaConfig{true, false, false, true};
    auto additonal_bccl_meta = torch_supa::profiler::impl::saveBcclMeta(fn, bcclMetaConfig);
    extra_meta.insert(additonal_bccl_meta.begin(), additonal_bccl_meta.end());
  }
#endif
  if (config.state == ProfilerState::KINETO_GPU_FALLBACK) {
    try {
      ...
      LOG(WARNING) << "Failed to record SUPA event. " << e.what();
    }
  } else if (config.state == ProfilerState::KINETO_PRIVATEUSE1_FALLBACK) {
    auto* fallback = kineto_ctx_ptr->fallback_;
    TORCH_INTERNAL_ASSERT(fallback != nullptr);
    torch::profiler::impl::privateuse1Stubs()->record(nullptr, &fallback->device_event_end_, nullptr);
  }

  if (fn.scope() == at::RecordScope::USER_SCOPE) {
    torch_supa::profiler::impl::kineto::popUserCorrelationId();
  } else {
    torch_supa::profiler::impl::kineto::popCorrelationId();
  }
}
...
void pushProfilingCallbacks(const std::unordered_set<at::RecordScope>& scopes) {
  ...
  auto recordFunctionCallback =
      at::RecordFunctionCallback(onFunctionEnter<use_global_callback>, onFunctionExit<use_global_callback>)
...
/*
  KinetoEvents
*/
KinetoEvent::KinetoEvent(const std::shared_ptr<const torch::profiler::impl::Result>& result, const bool verbose)
    : result_{result} {
  ...
  if (verbose) {
    ...
    while (parent != nullptr) {
      parent->visit_if_base<PyExtraFieldsBase>([&](const auto&) { python_stack_.push_back(parent->name()); });
      parent = parent->parent_.lock();
    }
  }
...
c10::ArrayRef<std::vector<int64_t>> KinetoEvent::shapes() const {
  return shapes_;
}
...
c10::ArrayRef<std::string> KinetoEvent::dtypes() const {
  return dtypes_;
}
...
c10::ArrayRef<c10::IValue> KinetoEvent::concreteInputs() const {
...
std::unordered_map<std::string, c10::IValue> KinetoEvent::kwinputs() const {
  return kwinputs_;
}

c10::ArrayRef<std::string> KinetoEvent::stack() const {
...
c10::ArrayRef<std::string> KinetoEvent::moduleHierarchy() const {
  auto const& extra_fields = result_->extra_fields_;
  if (const auto* p = std::get_if<ExtraFields<EventType::TorchOp>>(&extra_fields)) {
...
int KinetoEvent::deviceIndex() const {
  return result_->visit(c10::overloaded(
      [](const ExtraFields<EventType::Allocation>& i) { return static_cast<int>(i.device_index_); },
      [](const ExtraFields<EventType::OutOfMemory>& i) { return static_cast<int>(i.device_index_); },
...
int64_t KinetoEvent::supaElapsedUs() const {
  auto supa_event_start = fallbackStart();
  auto supa_event_end = fallbackEnd();
  if (!supa_event_start || !supa_event_end) {
    return -1;
  }
  try {
    // return (int64_t)torch::profiler::impl::cudaStubs()->elapsed(
    //     &cuda_event_start, &cuda_event_end);
    return (int64_t)torch::profiler::impl::privateuse1Stubs()->elapsed(&supa_event_start, &supa_event_end);
...
int64_t KinetoEvent::privateuse1ElapsedUs() const {
  ...
  return (int64_t)torch::profiler::impl::privateuse1Stubs()->elapsed(&privateuse1_event_start, &privateuse1_event_end);
  return -1;
}
...
#define FORWARD_FROM_RESULT(method_name, result_expr)                                              \
  decltype(std::declval<KinetoEvent>().method_name()) KinetoEvent::method_name() const {           \
    return static_cast<decltype(std::declval<KinetoEvent>().method_name())>(result_->result_expr); \
  }
...
#define TYPED_ATTR(event_type, method_name, expression) TYPED_ATTR_WITH_DEFAULT(event_type, method_name, expression, {})
...
TYPED_ATTR(TorchOp, flops, !e.extra_args_.empty() ? torch::profiler::impl::computeFlops(e.name_, e.extra_args_) : 0)
...
void prepareProfiler(
  ...
  auto const& config = supa_config.base_config;
  if (config.state == ProfilerState::NVTX || config.state == ProfilerState::ITT) {
    return;
  }
  TORCH_CHECK(
      config.state == ProfilerState::KINETO || config.state == ProfilerState::KINETO_GPU_FALLBACK ||
          config.state == ProfilerState::KINETO_PRIVATEUSE1_FALLBACK,
      "Supported only in Kineto profiler");
  torch_supa::profiler::impl::kineto::prepareTrace(
      /*cpuOnly=*/!c10::impl::hasDeviceGuardImpl(c10::DeviceType::PrivateUse1),
      activities,
  ...
  if (!config.experimental_config.performance_events.empty()) {
    ...
    TORCH_CHECK(
        activities.count(torch_supa::profiler::impl::SupaActivityType::CPU),
...
void toggleCollectionDynamic(
    const bool enable,
    const std::set<torch_supa::profiler::impl::SupaActivityType>& activities) {
  if (activities.count(torch_supa::profiler::impl::SupaActivityType::CPU) > 0 &&
      activities.count(torch_supa::profiler::impl::SupaActivityType::SUPA) == 0) {
    LOG(WARNING) << "Toggling CPU activity with SUPA activity on may result in "
                    "traces with SUPA events on artibrary tracks";
  } else if (
      activities.count(torch_supa::profiler::impl::SupaActivityType::SUPA) > 0 &&
      activities.count(torch_supa::profiler::impl::SupaActivityType::CPU) == 0) {
    LOG(WARNING) << "Toggling SUPA activity with CPU activity on may result in traces "
                    "with incorrect correlation between CPU and SUPA events";
  }
  for (auto act : activities) {
    if (act == torch_supa::profiler::impl::SupaActivityType::SUPA) {
      torch_supa::profiler::impl::kineto::toggleCollectionDynamic(enable);
    } else if (act == torch_supa::profiler::impl::SupaActivityType::CPU) {
      toggleCPUCollectionDynamic(enable);
    } else {
      LOG(WARNING) << "Dynamic toggle is only supported for CPU/GPU activity, "
...
void enableProfiler(
  ...
  const auto has_cpu = activities.count(torch_supa::profiler::impl::SupaActivityType::CPU);
  auto const& config = supa_config.base_config;
  TORCH_CHECK(
  ...
  TORCH_CHECK(
      config.state != ProfilerState::NVTX && config.state != ProfilerState::ITT &&
          config.state != ProfilerState::KINETO_PRIVATEUSE1_FALLBACK,
      "Not support NVTX, ITT, or KINETO_PRIVATEUSE1_FALLBACK");
  ...
  auto state_ptr = std::make_shared<KinetoThreadLocalState>(supa_config, activities);
  ...
  if (!config.global()) {
    torch_supa::profiler::impl::kineto::startTrace();
  }
...
std::unique_ptr<ProfilerResult> disableProfiler() {
  ...
  TORCH_CHECK(
      state_ptr &&
          (config.state == ProfilerState::KINETO || config.state == ProfilerState::KINETO_GPU_FALLBACK ||
           config.state == ProfilerState::KINETO_PRIVATEUSE1_FALLBACK ||
           config.state == ProfilerState::KINETO_ONDEMAND || config.state == ProfilerState::NVTX ||
  ...
  if (config.state == ProfilerState::KINETO || config.state == ProfilerState::KINETO_GPU_FALLBACK ||
      config.state == ProfilerState::KINETO_PRIVATEUSE1_FALLBACK) {
