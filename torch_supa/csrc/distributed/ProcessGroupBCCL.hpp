...
namespace c10d::supa {
// Control broadcasting of BCCL uniqueId
static std::vector<std::string> BCCL_BCAST_UNIQUEID = {"BCCL_BCAST_UNIQUEID", "TORCH_NCCL_BCAST_UNIQUEID"};

// Control whether to always use high priority streams
static std::vector<std::string> BCCL_HIGH_PRIORITY = {"BCCL_HIGH_PRIORITY", "TORCH_NCCL_HIGH_PRIORITY"};

// Control whether or not wait() is blocking or non-blocking.
static std::vector<std::string> BCCL_BLOCKING_WAIT = {
    "BCCL_BLOCKING_WAIT",
...
static std::vector<std::string> BCCL_ASYNC_ERROR_HANDLING = {
    "BCCL_ASYNC_ERROR_HANDLING",
...
static std::vector<std::string> BCCL_DUMP_ON_TIMEOUT = {"BCCL_DUMP_ON_TIMEOUT", "TORCH_NCCL_DUMP_ON_TIMEOUT"};

// Control whether Desync Debug is enabled. This variable must be set
// together with TORCH_NCCL_ASYNC_ERROR_HANDLING.
static std::vector<std::string> BCCL_DESYNC_DEBUG = {
    "BCCL_DESYNC_DEBUG",
    "TORCH_NCCL_DESYNC_DEBUG",
    "NCCL_DESYNC_DEBUG"};

// Enable recording start-events for all ProcessGroupBCCL collectives, and
...
static std::vector<std::string> BCCL_ENABLE_TIMING = {
    "BCCL_ENABLE_TIMING",
    "TORCH_NCCL_ENABLE_TIMING",
    "NCCL_ENABLE_TIMING"};

// Enable monitoring thread which aborts the process when the ProcessGroupBCCL
...
static std::vector<std::string> BCCL_ENABLE_MONITORING = {"BCCL_ENABLE_MONITORING", "TORCH_NCCL_ENABLE_MONITORING"};

// Control the watchdog heartbeat timeout period after which the monitoring
// thread will abort the process.
static std::vector<std::string> BCCL_HEARTBEAT_TIMEOUT_SEC = {
    "BCCL_HEARTBEAT_TIMEOUT_SEC",
    "TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC"};

// Whether to rethrow CUDA Errors in the watchdog (default true)
static std::vector<std::string> BCCL_RETHROW_CUDA_ERRORS = {
    "BCCL_RETHROW_SUPA_ERRORS",
...
static std::vector<std::string> BCCL_TRACE_BUFFER_SIZE = {"BCCL_TRACE_BUFFER_SIZE", "TORCH_NCCL_TRACE_BUFFER_SIZE"};

// Control how much extra time we will wait for dumping the debugging info
// before we exit and throws timeout exception.
static std::vector<std::string> BCCL_WAIT_TIMEOUT_DUMP_MILSEC = {
    "BCCL_WAIT_TIMEOUT_DUMP_MILSEC",
...
static std::vector<std::string> BCCL_COORD_CHECK_MILSEC = {"BCCL_COORD_CHECK_MILSEC", "TORCH_NCCL_COORD_CHECK_MILSEC"};

// Whether to log C++ stack traces on unclean shutdown (default true)
static std::vector<std::string> BCCL_LOG_CPP_STACK_ON_UNCLEAN_SHUTDOWN = {
    "BCCL_LOG_CPP_STACK_ON_UNCLEAN_SHUTDOWN",
...
// can cause a hang.
static std::vector<std::string> BCCL_SUPA_EVENT_CACHE = {"BCCL_SUPA_EVENT_CACHE", "TORCH_NCCL_CUDA_EVENT_CACHE"};

static std::vector<std::string> BCCL_NAN_CHECK = {"BCCL_NAN_CHECK", "TORCH_NCCL_NAN_CHECK"};

constexpr const char* BCCL_BACKEND_NAME = "bccl";

constexpr const char* EXCEPTION_DUMP = "exception_dump";

constexpr const int kWorkStatusUpdatePeriodMs = 30 * 1000; // 30 seconds

constexpr auto kProcessGroupBCCLDefaultTimeout = std::chrono::milliseconds(10 * 60 * 1000);

#if defined(__linux__)
struct DumpPipe {
  DumpPipe(int rank) {
    std::string fileStem = getCvarString({"BCCL_DEBUG_INFO_PIPE_FILE", "TORCH_NCCL_DEBUG_INFO_PIPE_FILE"}, "");
    if (fileStem.empty() || getCvarInt({"BCCL_TRACE_BUFFER_SIZE", "TORCH_NCCL_TRACE_BUFFER_SIZE"}, 0) <= 0) {
      return;
    }
    TORCH_CHECK(!fileStem.empty(), "BCCL_DEBUG_INFO_PIPE_FILE is empty");
    ...
    LOG(INFO) << "Pipe file " << filename << " has been opened, write to it to trigger BCCL Debug Dump.";
  ...
  DumpPipe(const DumpPipe&) = delete;
  DumpPipe& operator=(const DumpPipe&) = delete;
  DumpPipe(DumpPipe&&) = delete;
  DumpPipe& operator=(DumpPipe&&) = delete;
  ~DumpPipe() {
    if (fd_ != -1) {
      close(fd_);
    }
...
#endif
// ProcessGroupBCCL implements BCCL bindings for c10d.
//
// All functions of the class are expected to be called in the same order
// across all processes in the process group.  This is the only way that we
// can guarantee to match up the same calls among all processes.
//
// All BCCL functions provided by this class are asynchronous functions. More
// specifically, each BCCL call is scheduled on a separate SUPA stream that is
// different from the current SUPA stream. This is for the purpose of
// achieving potentially concurrency and better performance. As a result,
// it is the callers' responsibility to make sure that the SUPA stream their
// code works on needs to wait for the BCCL operation from
// this class.
//
// This can be done by calling:
//
// either WorkBCCL::wait() or WorkBCCL::synchronize(), both achieves the same
// functionality and are synonyms.
//
// Also note that WorkBCCL::finishedGPUExecution() is a helper function only
// provided by ProcessGroupBCCL to check if the BCCL operation of WorkBCCL has
// finished execution on the GPU (not just scheduled).
//
// Example on using the BCCL process group
//
//   ProcessGroupBCCL pg(store, rank, size);
//   std::shared_ptr<WorkBCCL> work = pg.allreduce(tensors);
//
//   // At this point, BCCL kernel has already by queued successfully
//   // Now, let current stream wait for the BCCL to finish, this function is
...
// NoHandling: do not handle asynchronous BCCL errors
// TearDown: tear down process upon error, see `WorkBCCL::handleException`
...
// removed in future) tear down process without cleaning up BCCL communicators.
// This should be used as a last resort in case `bcclCommAbort` itself is
// hanging
...
// If set, ProcessGroupBCCL doesn't use recordStream calls to ensure
...
static std::vector<std::string> BCCL_AVOID_RECORD_STREAMS = {
    "BCCL_AVOID_RECORD_STREAMS",
    "TORCH_NCCL_AVOID_RECORD_STREAMS"};

// If set, ProcessGroupBCCL registers postAlloc and preFree hooks to cuda cache
// allocator so that whenever a tensor is allocated or freed, ProcessGroupBCCL
// can register/deregister the tensor on all available NCCL communicators.
static std::vector<std::string> BCCL_USE_TENSOR_REGISTER_ALLOCATOR_HOOK = {
    "BCCL_USE_TENSOR_REGISTER_ALLOCATOR_HOOK",
...
class TORCH_API ProcessGroupBCCL : public Backend {
 public:
  class WorkBCCL : public Work, public std::enable_shared_from_this<WorkBCCL> {
   public:
    ...
    WorkBCCL(const WorkBCCL& w);
    WorkBCCL& operator=(const WorkBCCL&) = delete;
    WorkBCCL(WorkBCCL&&) = delete;
    WorkBCCL& operator=(WorkBCCL&&) = delete;

    ~WorkBCCL() override;

    // Checks if the BCCL kernel has started to execute.
    bool isStarted();

    // Checks if request has completed. In this specific case of BCCL, it checks
    // if the BCCL operation has completed on the GPU in its own BCCL stream.
    // Non-blocking operation.
    bool isCompleted() override;

    bool isSuccess() const override;

    // Same as calling synchronize() for BCCL work if timeout is not set.
    // Otherwise, it will block the CPU thread until the BCCL work is completed
    // or timed out. If timeout, exception will be thrown.
    bool wait(std::chrono::milliseconds timeout = kNoTimeout) override;

    void abort() override;

    // Let current stream wait on the completion of the BCCL work
    // Throws on exceptions.
    void synchronize() override;

    // Synchronize streams by blocking each on the BCCL stream
    void synchronizeStream();

    // Helper function to handle exception (throw if needed).
    void handleException(ErrorHandlingMode asyncErrorHandling);

    // Helper function that checks if the BCCL kernels have finished
    ...
    // Helper function that sets an exception_ptr on the WorkBCCL object.
    void setException(std::exception_ptr exception_ptr);

    // Helper function that returns True if the WorkBCCL object has timed out
    // and False otherwise.
    // In case of timeout, set exception on the WorkBCCL object.
   ...
   protected:
    ...
    // The start CUDA event of BCCL operator tracking this work item. These
    // start CUDA events are needed by desync debugging if enabled.
    std::shared_ptr<c10::supa::SUPAEvent> bcclStartEvent_;

    // The end CUDA event of BCCL operator tracking this work item.
    std::shared_ptr<c10::supa::SUPAEvent> bcclEndEvent_;

    // The BCCL communicator used for this work item.
    std::shared_ptr<BCCLComm> bcclComm_;

    // whether this work is a barrier op
    bool isBarrierOp_{false};

    // Clone of blockingWait_ from ProcessGroupBCCL.
    bool blockingWait_{false};

    // Whether or not to hook the cache allocator to register all allocated tensors
    bool useTensorRegisterAllocatorHook_ = false;

    // Clone of avoidRecordStreams_ from ProcessGroupBCCL.
    bool avoidRecordStreams_{false};

    // Clone of opTimeout_ from ProcessGroupBCCL.
    ...
    // Indicates if the bccl start event has been updated to the store trace.
    ...
    // Wrapper method for the static checkForBCCLErrors which can be overridden
    // for tests.
    virtual std::exception_ptr checkForBCCLErrors();

    friend std::ostream& operator<<(std::ostream& output, const WorkBCCL& workBCCL);

   private:
    // Helper function for synchronize
    void synchronizeInternal(std::chrono::milliseconds timeout);

    // Checks for BCCL errors and sets an appropriate exception_ptr.
    ...
    // Store a reference to BCCL collective's outputs, used by result and to
    ...
    // stream(s) are synced with the bccl work stream(s).
  ...
  class SUPAEventCache {
   ...
   private:
    ...
    std::array<std::deque<c10::supa::SUPAEvent*>, 2> eventsArray_; // 0 for timing=false, 1 for timing=true
  };

  struct Options : Backend::Options {
    // NOTE: timeout in ProcessGroupBCCL::Options denote the timeout for
    ...
    static c10::intrusive_ptr<Options> create(bool is_high_priority_stream = false) {
      return c10::make_intrusive<Options>(is_high_priority_stream);
    }

    // Schedule BCCL operations on high priority CUDA streams
    bool is_high_priority_stream;

#ifdef BCCL_HAS_COMM_NONBLOCKING
    // Configure ranks
    bcclConfig_t config = BCCL_CONFIG_INITIALIZER;
#endif

    // Optional "parent" backend and color to create communicators from
    // via `bcclCommSplit`
    c10::intrusive_ptr<ProcessGroupBCCL> split_from;
    // Color to use for `bcclCommSplit`, values:
    // * Non-negative value: in group;
    // * BCCL_SPLIT_NOCOLOR (-1): not in group;
    // * BCCL_SPLIT_NOCOLOR - 1: uninitialized.
    // [Note 1]: the type must be `int` instead of `int64_t` because BCCL API
    ...
    int split_color{BCCL_SPLIT_NOCOLOR - 1};
#else
  ...
  class DesyncDebugger {
   public:
    ...
    void logWorkStart(WorkBCCL& work);

    // Log work end to store.
    void logWorkEnd(WorkBCCL& work);
   ...
   private:
    ...
    // The store keys to trace the last BCCL collective kernel CUDA events -
  ...
  // doesn't create any BCCL communicators. A single BCCL communicator can
  // only be used on a specific set of devices, and are therefore created
  // on-demand when a collective runs. If another collective is executed later,
  // against a different set of devices, the process group creates another BCCL
  // communicator. These BCCL communicators are cached and reused if possible.
  //
  ProcessGroupBCCL(
  ...
  C10_DEPRECATED ProcessGroupBCCL(
      const c10::intrusive_ptr<Store>& store,
      int rank,
      int size,
      const std::string& groupName,
      c10::intrusive_ptr<Options> options = Options::create())
      : ProcessGroupBCCL(store, rank, size, std::move(options)) {}

  ProcessGroupBCCL(const ProcessGroupBCCL&) = delete;
  ProcessGroupBCCL& operator=(const ProcessGroupBCCL&) = delete;
  ProcessGroupBCCL(ProcessGroupBCCL&&) = delete;
  ProcessGroupBCCL& operator=(ProcessGroupBCCL&&) = delete;
  ~ProcessGroupBCCL() override;

  // This function returns a local uid for ProcessGroupBCCL.
  ...
  const std::string getBackendName() const override {
    return std::string(BCCL_BACKEND_NAME);
  }
  ...
  void groupEndNonblocking(const std::shared_ptr<BCCLComm>& comm);
  ...
  // group have performed.  Counts bcclCommCreateFromRanks()
  uint64_t getCommSplitCounter() const;

  void registerOnCompletionHook(std::function<void(std::shared_ptr<WorkInfo>)>&& hook) override;
  ...
  // Provides an API to abort the ProcessGroup (similar to bcclCommAbort)
  // instead of relying on ProcessGroupBCCL destructor.
#if TORCH_VER >= TORCH_2_7_0
  void abort() override;
#else
  void abort();
  ...
  // Performs BCCL user buffer registration for all buffers in
  // the given MemPool
  void registerMemPool(c10::supa::MemPool* pool, bool symm = false);

  // Performs BCCL user buffer de-registration for all buffers in
  // the given MemPool
  void deregisterMemPool(c10::supa::MemPool* pool);
  ...
  // want to expose the `WorkBCCL` via pybind. It verifies whether the
  // `opTimeout_` of the provided WorkBCCL instance is the same as the specified
  // timeout.
  static bool verifyWorkTimeoutForTest(const c10::intrusive_ptr<Work>& work, const std::chrono::milliseconds& timeout);

 protected:
  // Helper that broadcasts bccl unique ID to all ranks through the store
  void broadcastUniqueBCCLID(bcclUniqueId* bcclID, bool isSingleP2POp, const std::string& devicesKey, int p2pRank);

  // Helper that looks up the cached BCCL communicators only
  std::shared_ptr<BCCLComm> getBCCLComm(const std::string& deviceKey);

  std::shared_ptr<BCCLComm> initBCCLComm(
  ...
  virtual std::exception_ptr checkForBCCLErrors(std::shared_ptr<BCCLComm>& bcclComm);

  // Ensure thaht if record is True, the work obj will be enqueued via
  // workEnqueue
  virtual c10::intrusive_ptr<ProcessGroupBCCL::WorkBCCL> initWork(
      at::Device& device,
      int rank,
      OpType opType,
      bool isP2P,
  ...
  // In the timeout case and we will dump debug info such as the BCCL flight
 ...
 private:
  ...
  //
  //    bcclResult_t fn(at::Tensor& input, at::Tensor& output,
  //                    bcclComm_t, at::cuda::CUDAStream&);
  ...
  c10::intrusive_ptr<Work> allreduce_impl(
      at::Tensor& tensor,
      const char* profilingTitle = "bccl:all_reduce",
      const AllreduceOptions& opts = AllreduceOptions());

  // Checks for BCCL errors on each of the communicators and returns an
  // appropriate exception_ptr (nullptr if no errors).
  static std::exception_ptr checkForBCCLErrorsInternal(std::shared_ptr<BCCLComm>& bcclComm);

  // Function that runs as part of a separate thread and checks for errors on
  // BCCL communicators. We need a separate thread to check for BCCL errors
  // since we can't rely on the user calling certain methods like wait(),
  // isCompleted() etc. to detect and remediate errors. In addition to this, we
  // need a mechanism to safely abort and remove BCCL communicators from our
  // cache. This can be done cleanly by having a thread for the ProcessGroupBCCL
  // class. Attempting to modify the communicator cache from the WorkBCCL class
  // might run into issues with object lifetime since the ProcessGroupBCCL
  // object might get destroyed before the WorkBCCL object.
  void bcclCommWatchdog();
  ...
  // Destroys initialized BCCL communicators in devBCCLComMap_ given by input
  ...
  void destroyBCCLComms(const std::string& devBCCLCommMapKey);
  ...
  // devices. It is called in the constructor of ProcessGroupBCCL, so it always
 ...
 protected:
  ...
  // so that when we get stuck in some BCCL/CUDA calls,
  ...
  std::string getBCCLWatchdogTimeoutErrorMsg(const std::string& extraMsg);

  std::string getBCCLWatchdogTimeoutExitMsg(const std::string& exitReason);

  static const int64_t kWatchdogThreadSleepMillis;

  // The store is used to broadcast the BCCL unique ID of rank 0. This store
  // comes with prefix and it is different across ProcessGroup BCCL instances
  // (aka, different ProcessGroups).
  c10::intrusive_ptr<Store> store_;

  // Reference to the store without prefix so that keys are same across all
  // ProcessGroup BCCL instances and (key, value) pairs written to the store are
  // global.
  ...
  std::chrono::milliseconds ephemeralTimeoutActive_ = std::chrono::milliseconds(0);

  // The ephemeral timeout addition which has been already applied to work.
  std::chrono::milliseconds ephemeralTimeoutInflight_ = std::chrono::milliseconds(0);

  const c10::intrusive_ptr<Options> options_;

  // The number of BCCL communicators that have been created during
  // the lifetime of this process group. This sequence number is
  // used to scope keys used in the store.
  uint64_t bcclCommCounter_{0};

  // The BCCL communicator that the process group has cached.
  //
  // For collective operations:
  // The key is a list of GPU devices that an operation is operating on
  // The GPU devices are stored in a device sequence and the cache BCCL
  ...
  std::unordered_map<std::string, std::shared_ptr<BCCLComm>> devBCCLCommMap_;

  // The BCCL communicators currently in process of being initialized.
  std::unordered_map<std::string, std::shared_ptr<BCCLComm>> inInitializationCommMap_;

  // Mutex to guard maps like devBCCLCommMap_.
  std::mutex mutex_;
  ...
  // Interval of check coordinated signals in ProcessGroupBCCL from other ranks
  // e.g., trigger the dump of the debugging info for timeout when notified.
  int coordCheckIntervalMilSec_;

  // Size of ring buffer where we store BCCL Traces for debugging.
  ...
  std::atomic<bool> supaEventCacheEnabled_{};
  ...
  std::thread bcclHeartbeatMonitorThread_;

  // Watchdog thread which looks for errors on the cached BCCL communicators.
  std::thread bcclCommWatchdogThread_;
  ...
  // Vector to Store WorkBCCL pointers
  std::list<ProcessGroupBCCL::WorkBCCL> workMetaList_;
  ...
  std::list<ProcessGroupBCCL::WorkBCCL> completedWorkList_;

  // Add Work Pointer to workVector
  void workEnqueue(const c10::intrusive_ptr<ProcessGroupBCCL::WorkBCCL>& /*work*/);

  // The CUDA streams used by BCCL kernels
  std::unordered_map<std::string, c10::supa::SUPAStream> bcclStreams_;

  // The CUDA events used to sync BCCL streams
  std::unordered_map<std::string, c10::supa::SUPAEvent> bcclEvents_;
  ...
  std::shared_ptr<BCCLComm> coalescedComm_ = nullptr;
  ...
  // Whether the BCCL watchdog should rethrow CUDA errors.
  bool rethrowSUPAErrors_ = false;

  // The number of active bcclGroupStart() calls. This counter will be increased
  // by 1 when bcclGroupStart() is called and decreased by 1 when bcclGroupEnd()
  // is called.
  static thread_local uint64_t bcclActiveGroupCounter_;

  // Counting for the sequential number of BCCL collective call.
  // (specifically, how many actual kernels we launched, which differs from
  // op_id_ when coalescing is enabled)
  uint64_t seqCollective_{0};

  // Counting for the sequential number of BCCL P2P calls.
  uint64_t seqP2P_{0};
  ...
  // The number of ProcessGroupBCCL created on the current rank.
  size_t local_id_;
  ...
  // Internal cached value: use BCCL non-blocking API mode or not.
  ...
  std::unique_ptr<c10::supa::MemPool> memPool_ = nullptr;
};

// Dumps the BCCL comm traces and additional information about the Process
// Group.
TORCH_API std::string dump_bccl_trace(bool includeCollectives, bool includeStackTraces, bool onlyActive);

// Dumps the BCCL comm traces and additional information about the Process
// Group in JSON formatted string.
// We don't include stack traces in JSON format as it is far too much data.
TORCH_API std::string dump_bccl_trace_json(bool includeCollectives, bool onlyActive);
...
TORCH_API std::optional<std::function<void(std::function<void(const std::string&)>)>>& get_cpp_trace_dumper();
