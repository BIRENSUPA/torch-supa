...
bool device_has_multicast_support(int device_idx) {
  ...
  return c10d::supa::deviceSupportsMulticast(device_idx);
}

bool allow_overlapping_devices() {
  return c10::utils::check_env("TORCH_SYMM_MEM_ALLOW_OVERLAPPING_DEVICES") == true;
}
...
std::string getSymmMemBackendSUPA() {
  ...
  // backend.
  static auto val = torch_supa::utils::get_env("TORCH_SYMMMEM");
  if (val.has_value()) {
    TORCH_CHECK(
        val.value() == "SUPA" || val.value() == "BRSHMEM" || val.value() == "BCCL",
        "TORCH_SYMMMEM environment variable must be one of 'SUPA', 'BRSHMEM', 'BCCL'.")
    return val.value();
  }
  return "SUPA";
}

IpcChannel::IpcChannel() : socket_name_(get_socket_name(getpid())), socket_(socket(AF_UNIX, SOCK_DGRAM, 0)) {
  // On success, a file descriptor for the new socket is returned.
  //  On error, -1 is returned, and errno is set to indicate the error.
  TORCH_CHECK(socket_ != -1, "Failed to create socket: ", c10::utils::str_error(errno));
...
void IpcChannel::send_fd(int dst_pid, int fd) const {
  ...
  struct msghdr msg {
    ...
    .msg_name = (void*)&addr, .msg_namelen = sizeof(struct sockaddr_un), .msg_iov = &io, .msg_iovlen = 1,
  ...
  TORCH_CHECK(sendmsg(socket_, &msg, 0) > 0, "Failed to send fd: ", c10::utils::str_error(errno));
}
...
int IpcChannel::recv_fd() {
  ...
  TORCH_CHECK(recvmsg(socket_, &msg, 0) > 0, "Failed to receive fd: ", c10::utils::str_error(errno));
...
std::vector<int> IpcChannel::all_gather_fds(int rank, const std::vector<int>& pids, int fd) {
...
int IpcChannel::broadcast_fds(int rank, int src_rank, const std::vector<int>& pids, int fd) {
  int world_size = static_cast<int>(pids.size());
...
void map_block(void** ptr, c10d::supa::symmetric_memory::HandleType handle, size_t size, int device_idx) {
  ...
  auto* dev_ptr = reinterpret_cast<SUdeviceptr*>(ptr);
  // Allocate virtual address space
  C10_SUPA_DRIVER_CHECK(driver_api->suMemAddressReserve_(dev_ptr, size, 0ULL, 0, 0ULL));
  // Map the physical memory to the virtual address
  C10_SUPA_DRIVER_CHECK(driver_api->suMemMap_(*dev_ptr, size, 0, handle, 0ULL));
  ...
  C10_SUPA_DRIVER_CHECK(driver_api->suMemSetAccess_(*dev_ptr, size, &desc, 1));
}
