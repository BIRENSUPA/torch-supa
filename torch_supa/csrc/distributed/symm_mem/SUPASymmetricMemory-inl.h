...
#define BRCC_SUPPORTS_MULTICAST 1
...
#include "torch_supa/csrc/aten/ops/kernels/elementwise/MemoryAccess.cuh"
...
template <class... T>
inline constexpr bool dependent_false = at::native::memory::dependent_false<T...>;
...
#ifdef __MIRA__
  ::supa::atomic_ref<uint32_t, ::supa::thread_scope_system> ref(*addr);
  ref.compare_exchange_strong(compare, val, ::supa::std::memory_order(Sem));
  return compare;
#else
...
__device__ __forceinline__ void sync_remote_blocks(uint32_t** signal_pads, size_t rank, size_t world_size) {
  if (threadIdx.x < world_size) {
    ...
    if constexpr (hasPrevMemAccess) {
      put_signal<std::memory_order_release>(signal_pads[target_rank] + blockIdx.x * world_size + rank);
    } else {
      put_signal<std::memory_order_relaxed>(signal_pads[target_rank] + blockIdx.x * world_size + rank);
    }
    if constexpr (hasSubsequentMemAccess) {
      wait_signal<std::memory_order_acquire>(signal_pads[rank] + blockIdx.x * world_size + target_rank);
    } else {
      wait_signal<std::memory_order_relaxed>(signal_pads[rank] + blockIdx.x * world_size + target_rank);
    }
  }
};
...
#if defined(BRCC_SUPPORTS_MULTICAST)
...
__device__ __inline__ T add_bf16x2(T a, T b) {
  ...
  auto res = __hadd2(*reinterpret_cast<__br_bfloat162*>(&a), *reinterpret_cast<__br_bfloat162*>(&b));
...
__device__ inline std::enable_if_t<(k_world_size > 0), Vec<alignment>> load_and_reduce(
    T** ptrs,
    size_t rank,
    size_t world_size,
    size_t offset) {
...
#pragma unroll k_world_size
  for (size_t step = 0; step < k_world_size; ++step) {
    ...
    vecs[remote_rank] = at::native::memory::ld_vec<alignment>(ptrs[remote_rank] + offset);
  }
  auto acc = vecs[0];
