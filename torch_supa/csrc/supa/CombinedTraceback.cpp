...
namespace {
std::mutex to_free_frames_mutex;
std::vector<CapturedTraceback::PyFrame> to_free_frames;
  ...
  void appendSymbolized(const std::vector<CapturedTraceback::PyFrame>& to_symbolize, SymbolizedTracebacks& result)
    ...
    for (const auto& f : to_symbolize) {
      ...
      result.all_frames.emplace_back(
          unwind::Frame{py::cast<std::string>(filename), py::cast<std::string>(funcname), (uint64_t)lineno});
...
void installCapturedTracebackPython() {
  CapturedTraceback::addPythonUnwinder(new PythonTraceback()); // NOLINT(cppcoreguidelines-owning-memory)
}
