...
using namespace at;
using namespace torch_supa;
using namespace torch_supa::supa::bccl;
using namespace torch_supa::supa::bccl::detail;

static const char* COMM_CAPSULE_NAME = "torch_supa.supa.bccl.Communicator";
...
PyObject* THBPModule_bccl_unique_id(PyObject* self, PyObject* args) {
  ...
  bcclUniqueId id;
  get_unique_id(id);
  return PyBytes_FromStringAndSize((char*)&id, BCCL_UNIQUE_ID_BYTES);
  END_HANDLE_TH_ERRORS
}

static bcclComm_t unpack_bccl_comm(PyObject* capsule) {
  bcclComm_t comm = (bcclComm_t)PyCapsule_GetPointer(capsule, COMM_CAPSULE_NAME);
  if (!comm) {
    throw python_error();
  }
  return comm;
...
static void destroy_bccl_comm(PyObject* capsule) {
  HANDLE_TH_ERRORS
  bcclComm_t comm = unpack_bccl_comm(capsule);
  {
...
static std::vector<std::optional<c10::supa::SUPAStream>> unpack_streams(PyObject* obj, size_t size) {
  if (obj == Py_None) {
    return std::vector<std::optional<c10::supa::SUPAStream>>(size, std::nullopt);
  }
  auto streams = THPUtils_PySequence_to_SUPAStreamList(obj);
...
static std::vector<bcclComm_t> unpack_comms(PyObject* obj, size_t size) {
  if (obj == Py_None) {
    return std::vector<bcclComm_t>();
  }
  std::vector<bcclComm_t> comms;
  if (PyCapsule_CheckExact(obj)) {
    comms = {unpack_bccl_comm(obj)};
  } else {
    ...
    comms = std::vector<bcclComm_t>(size);
    for (const auto i : c10::irange(size)) {
      comms[i] = unpack_bccl_comm(PySequence_Fast_GET_ITEM(seq.get(), i));
    }
  }
  if (comms.size() != size) {
...
PyObject* THBPModule_bccl_init_rank(PyObject* self, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(args, "is#i:bccl_init_rank", &nranks, &id, &id_len, &rank)) {
    return nullptr;
  }
  TORCH_CHECK(
      id_len == BCCL_UNIQUE_ID_BYTES,
      "invalid unique_id (expected ",
      BCCL_UNIQUE_ID_BYTES,
      " bytes, got ",
      id_len,
      ")");

  bcclUniqueId commId;
  memcpy(&commId, id, BCCL_UNIQUE_ID_BYTES);
  bcclComm_t comm = nullptr;
  {
    pybind11::gil_scoped_release no_gil;
    comm = comm_init_rank(nranks, commId, rank);
  }
  return PyCapsule_New(comm, COMM_CAPSULE_NAME, &destroy_bccl_comm);
  END_HANDLE_TH_ERRORS
}

PyObject* THBPModule_bccl_reduce(PyObject* self, PyObject* args) {
  HANDLE_TH_ERRORS
  PyObject* _inputs = nullptr;
  PyObject* _output = nullptr;
  PyObject* _streams = nullptr;
  PyObject* _comms = nullptr;
  int root = 0;
  int op = 0;

  if (!PyArg_ParseTuple(args, "OOiiOO", &_inputs, &_output, &root, &op, &_streams, &_comms)) {
    THPUtils_invalidArguments(
        ...
        " int op, sequence[torch.supa.Stream or None]");
    return nullptr;
  }

  std::vector<at::Tensor> inputs = extract_tensors(_inputs);
  auto output = extract_tensor(_output);
  std::vector<std::optional<c10::supa::SUPAStream>> streams = unpack_streams(_streams, inputs.size());
  auto user_comms = unpack_comms(_comms, inputs.size());

  {
    pybind11::gil_scoped_release no_gil;
    torch_supa::supa::bccl::reduce(inputs, output, root, op, streams, user_comms);
  }
...
PyObject* THBPModule_bccl_all_reduce(PyObject* self, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(args, "OOiOO", &_inputs, &_outputs, &op, &_streams, &_comms)) {
    THPUtils_invalidArguments(
        ...
        " sequence[torch.supa.Stream] streams,"
        " sequence[torch.supa.bccl.Communicator] comms)");
    return nullptr;
  }
...
PyObject* THBPModule_bccl_broadcast(PyObject* self, PyObject* args) {
  ...
  {
    pybind11::gil_scoped_release no_gil;
    torch_supa::supa::bccl::broadcast(inputs, streams, user_comms);
  }
...
static at::Tensor extract_tensor(PyObject* obj) {
  TORCH_CHECK_TYPE(THPVariable_Check(obj), "expected Tensor (got ", Py_TYPE(obj)->tp_name, ")");
...
static std::vector<at::Tensor> extract_tensors(PyObject* obj) {
  ...
  for (Py_ssize_t i = 0; i < length; i++) {
    ...
    TORCH_CHECK_TYPE(THPVariable_Check(item), "expected Tensor at ", i, " (got ", Py_TYPE(item)->tp_name, ")");
