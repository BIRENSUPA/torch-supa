...
#include "torch_supa/csrc/profiler/record_function_wrapper.h"
...
c10::List<IValue> new_list() {
  return c10::List<IValue>(c10::AnyType::get());
}

std::vector<IValue> ivalue_symbolize(std::vector<torch::CapturedTraceback*>& to_symbolize) {
...
//               c10::supa::SUPACachingAllocator::recordAnnotation(
...
void checkOptionIn(const std::string& option, std::initializer_list<std::string> valid, const char* error) {
  TORCH_CHECK(valid.end() != std::find(valid.begin(), valid.end(), option), error);
}
...
void _record_memory_history(
  ...
  c10::supa::SUPACachingAllocator::CreateContextFn recorder = gather;
  if (enabled && record_cpp_context && (trace_alloc_record_context || record_context)) {
    recorder = gather_with_cpp;
    // warm up C++ stack unwinding
    torch::unwind::unwind();
  }
  auto when = c10::supa::SUPACachingAllocator::RecordContext::NEVER;
  if (trace_alloc_record_context) {
    when = c10::supa::SUPACachingAllocator::RecordContext::ALLOC;
  } else if (record_context) {
    when = c10::supa::SUPACachingAllocator::RecordContext::STATE;
  }
  c10::supa::SupaSysCtrl::GetInstance().supaInit();
  // _initRecordAnnotations();
  c10::supa::SUPACachingAllocator::recordHistory(enabled, recorder, trace_alloc_max_entries, when);
}
...
void _record_memory_history(
  ...
  if (context) {
    if (context == "all") {
      when = c10::supa::SUPACachingAllocator::RecordContext::ALL;
  ...
  c10::supa::SupaSysCtrl::GetInstance().supaInit();
  // _initRecordAnnotations();
  c10::supa::SUPACachingAllocator::recordHistory(enabled.has_value(), recorder, max_entries, when);
}
...
std::string _memory_snapshot_pickled() {
  ...
  // IValue last_allocator_settings_s = "PYTORCH_SUPA_ALLOC_CONF";
  ...
  IValue release_lock_on_malloc_s = "release_lock_on_supamalloc";
  IValue pinned_use_host_register_s = "pinned_use_supa_host_register";
  ...
  allocator_settings.insert(max_split_size_s, int64_t(snapshot.config_metadata.max_split_size));
  allocator_settings.insert(garbage_collection_threshold_s, snapshot.config_metadata.garbage_collection_threshold);
  allocator_settings.insert(expandable_segments_s, snapshot.config_metadata.expandable_segments);
  allocator_settings.insert(
      pinned_num_register_threads_s, int64_t(snapshot.config_metadata.pinned_num_register_threads));
  allocator_settings.insert(release_lock_on_malloc_s, snapshot.config_metadata.release_lock_on_malloc);
  allocator_settings.insert(pinned_use_host_register_s, snapshot.config_metadata.pinned_use_host_register);
...
void _record_memory_history(
    ...
    bool trace_alloc_record_context,
    bool record_cpp_context) {
  TORCH_CHECK(false, "supa did not support _record_memory_history for torch <= 2.3");
}

void _record_memory_history(
    c10::optional<std::string> enable,
...
std::string _memory_snapshot_pickled() {
  TORCH_CHECK(false, "supa did not support _memory_snapshot_pickled for torch <= 2.3");
}
#endif
