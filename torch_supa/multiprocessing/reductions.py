...
__all__ = ["rebuild_supa_event", "rebuild_supa_tensor"]
...
def rebuild_supa_event(device, handle):
    return torch.supa.Event.from_ipc_handle(device, handle)


def _supa_reduce_event(event):
    handle = event.ipc_handle()
    return (rebuild_supa_event, (event.device, handle))


def rebuild_supa_tensor(
    tensor_cls,
    tensor_size,
    tensor_stride,
    tensor_offset,
    ...
    if storage_handle is None or storage_size_bytes == 0:
        ...
        if storage is None:
            ...
            storage = storage_cls._new_shared_supa(
                storage_device,
                storage_handle,
                storage_size_bytes,
                storage_offset_bytes,
            ...
            storage_cls._release_ipc_counter_supa(
...
def _supa_reduce_tensor(tensor):
    ...
    if storage._untyped_storage.device.type == "supa":
        (
            device,
            handle,
            storage_size_bytes,
...
def _supa_reduce_storage(storage):
...
def _add_reductions_methods():
    multiprocessing.reduction.register(torch.supa.Event, _supa_reduce_event)
    torch.multiprocessing.reductions.reduce_event = _supa_reduce_event
    torch.multiprocessing.reductions.reduce_tensor = _supa_reduce_tensor
    torch.multiprocessing.reductions.reduce_storage = _supa_reduce_storage
