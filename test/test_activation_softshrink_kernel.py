...
class TestSoftShrink:
    ...
    def test_softshrink(self, input_shape, dtype, requires_grad):
        cpu_input, supa_input = create_random_tensor(
            input_shape, dtype=dtype, requires_grad=requires_grad
        )

        cpu_out = torch.nn.functional.softshrink(cpu_input)
        supa_out = torch.nn.functional.softshrink(supa_input)
