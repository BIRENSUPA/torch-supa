    ...
    def _test_multiple_optimizers(self, sharding_strategy: ShardingStrategy):
        ...
        for (ddp_n, ddp_p), (fsdp_n, fsdp_p) in zip(
            ...
            if ddp_p.grad is None:
                ...
                # NOTE(DIS-1612): Skipped -- FSDP returns sharded views (shape reflects local shard
                # size, not original param size) while DDP returns full params. Comparison requires
                # `FSDP.summon_full_params()` context to unshard, which is already done in
                # `_check_ddp_fsdp_param_parity()` below. The shape mismatch is expected when
                # `use_orig_params=True` with sharding strategies due to alignment padding (PR #97667).
                pass
