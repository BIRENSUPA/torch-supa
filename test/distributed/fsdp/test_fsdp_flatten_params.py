...
class TestFlattenParams(FSDPTest):
    ...
    def _get_transformer(self, seed=0):
        ...
        def get_input(device, dtype):
            ...
            src = torch.rand(2, 2, 4).to(device=device, dtype=dtype)  # T x B x C
            tgt = torch.rand(1, 2, 4).to(device=device, dtype=dtype)  # T x B x C
            return (src, tgt)
    ...
    def _test_partial_flattening(self, half: bool):
        ...
        encoder_1_params = list[Parameter](module.encoder.layers[1].parameters())
