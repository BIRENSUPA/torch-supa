...
class TestFSDPMiscMultiProcess(FSDPTest):
    ...
    def test_fsdp_zero2_eval_with_prefetch(self):
        ...
        grads = []
        for i in range(2):
            x = torch.randn(2, 1, 8, 8, device="cuda").requires_grad_()
            y = torch.randint(low=0, high=2, size=(2,), device="cuda")
...
class TestFSDPMiscMultiThread(FSDPTestMultiThread):
    ...
    @pytest.mark.gcuStress
    @skip_if_lt_x_gpu(2)
    @unittest.skip(reason="TODO(DIS-1726): failed due to gpu trace not support clear")
    def test_module_device_mismatches_device_id(self):
        ...
        torch.cuda.set_device(self.rank)
        context = (
            self.assertRaisesRegex(ValueError, f"supa:{self.rank} vs supa:0")
            if self.rank != 0
            else nullcontext()
        )
        with context:
        ...
        context = (
            (
                self.assertRaisesRegex(
                    ValueError,
                    f"Inconsistent.*supa:{self.rank} vs supa:0",
                )
            )
            if self.rank != 0
            else nullcontext()
    ...
    @pytest.mark.gcuStress
    @skip_if_lt_x_gpu(2)
    @unittest.skip(reason="TODO(DIS-1733): random attribute error: 'list' object has no attribute 'to', wait for fix")
    ...
    @pytest.mark.gcuStress
    @skip_if_lt_x_gpu(2)
    @unittest.skip(reason="TODO(DIS-1780): random AssertionError: UserWarning not triggered")
    def test_fsdp_unsupported_module_cls(self):
        ...
        model = nn.ModuleList([MLP(8, torch.device("cpu")) for _ in range(2)])
...
class TestFSDPMiscWorldSize1(FSDPTestMultiThread):
    ...
    def test_training_device_mismatch_errors(self):
        ...
        with self.assertRaisesRegex(
            ...
            "sure to move the module to supa:0 before training.",
        ):
            fsdp_model(inp)
        ...
        with self.assertRaisesRegex(
            ...
            "parameters on supa:0. Make sure to not move the module from CPU "
