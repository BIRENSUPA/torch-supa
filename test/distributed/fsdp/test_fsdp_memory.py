...
class TestFSDPMemory(FSDPTest):
    ...
    def _dist_train(self, with_checkpoint, expected, model_hidden_dim, iterations):
        gpu_id = self.rank
        batch = torch.randn(size=(2, 3, 32, 32)).cuda()
    ...
    def test_fsdp_memory(self, ckpt):
        # hidden_dim 16: model size ~0.5MB
        model_hidden_dim = 16
        ...
        for iteration in range(iterations):
            if iteration == 0:
                ...
                if ckpt == "ckpt":
                    expected[f"iter {iteration}: after fwd"] = 1
                    expected[f"iter {iteration}: after loss"] = 1
                else:
                    expected[f"iter {iteration}: after fwd"] = 2
                    expected[f"iter {iteration}: after loss"] = 2
