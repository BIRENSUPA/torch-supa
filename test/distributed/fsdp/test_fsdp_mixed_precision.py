...
import torch_supa.testing  # noqa
...
_BUFFER_ORIG_DTYPE = torch.float32
...
class LinearMixedPrecision(nn.Module):
    ...
    def __init__(self, param_dtype, buffer_name="buffer", run_checks=True):
        super().__init__()
        self.lin = nn.Linear(4, 4, bias=False).to(param_dtype)
    ...
    def _run_test_mixed_precision_e2e(
        ...
        for model in fsdp_models:
            ...
            with patch_reduce_scatter(test_reduce_scatter, full_precision_param_dtype):
                ...
                for _ in range(2):
                    inp = torch.randn(
                        3, 4, device="cuda", dtype=full_precision_param_dtype
                    )
...
class TestFSDPMixedPrecisionSharded(TestFSDPMixedPrecision):
    ...
    def test_mixed_precision_no_reshard_after_forward(self):
        ...
        self._run_test_mixed_precision_e2e(
            ...
            full_precision_param_dtype=torch.float32,
    ...
    @pytest.mark.gcuStress
    @skip_if_lt_x_gpu(2)
    @unittest.skip(reason="TODO(DIS-1521): Skip for so much computation on CModel")
    @skipIfNoTorchVision
    ...
    def test_mp_batchnorm(self, convert_sync_bn):
        class BatchNormNet(nn.Module):
            ...
            def forward(self, x):
                x = torch.reshape(self.fc1(x), (-1, 4, 2))
                x = self.bn(x)
    ...
    def test_full_precision_in_eval_buffers(self):
        ...
        for (
            ...
            inp = torch.randn(3, 4, device="cuda")
    ...
    def test_buffer_dtype_no_root_handle(self):
        ...
        model = nn.Sequential(
            nn.Sequential(nn.Conv2d(4, 4, 3, padding=1)),
            nn.Sequential(NonLearnableConv((1, 2, 2, 1), 4)),
            nn.Sequential(nn.Conv2d(4, 4, 3, padding=1)),
            nn.Sequential(NonLearnableConv((1, 2, 2, 1), 4)),
        ).cuda()
        ...
        x = torch.randn(4, 4, 8, 8, device="cuda")
        out = model(x)
        out.mean().backward()
...
class TestFSDPMixedPrecisionIgnoredModules(FSDPTest):
    ...
    def test_mixed_precision_with_ignored_module(self):
        ...
        x = torch.ones(2, 4, device=torch.cuda.current_device())
    ...
    def test_submodules_with_external_inputs(self):
        ...
        class ToyModel(nn.Module):
            ...
            def forward(self, x: torch.Tensor) -> torch.Tensor:
                ...
                y = torch.ones(2, 4, device="cuda", dtype=torch.float32)
        ...
        x = torch.zeros(2, 4, device="cuda", dtype=torch.float32)
...
class TestFSDPTrainEval(FSDPTest):
    ...
    def _test_train_ema_eval_flow(self, sharding_strategy: ShardingStrategy):
        ...
        eval_src = torch.randn((4, 4, 4), device=device)
        eval_tgt = torch.randn((4, 4, 4), device=device)
        ...
        for _ in range(2):
            fsdp_model.train()
            train_src = torch.randn((4, 4, 4), device=device)
            train_tgt = torch.randn((4, 4, 4), device=device)
