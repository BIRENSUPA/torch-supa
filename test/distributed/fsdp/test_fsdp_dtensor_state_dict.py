...
class TestDummyModel(torch.nn.Module):
    def __init__(self):
        super().__init__()
        torch.manual_seed(0)
        self.net1 = nn.Sequential(nn.Linear(4, 4), nn.ReLU())
        self.net2 = nn.Sequential(nn.Linear(4, 4), nn.ReLU())
        self.net3 = nn.Sequential(nn.Linear(4, 4), nn.ReLU())
        self.net4 = nn.Sequential(nn.ReLU(), nn.Linear(4, 4))

    def forward(self, x):
        return self.net4(self.net3(self.net2(self.net1(x))))

    def get_input(self):
        return torch.rand(4, 4, device=device_type.type)
...
class TestDummyModelUneven(torch.nn.Module):
    def __init__(self):
        super().__init__()
        torch.manual_seed(0)
        self.net1 = nn.Sequential(nn.Linear(5, 4), nn.ReLU())
        self.net2 = nn.Sequential(nn.Linear(4, 4), nn.ReLU())
        self.net3 = nn.Linear(4, 4)
        self.net4 = nn.Sequential(nn.ReLU(), nn.Linear(4, 5))
...
class TestFSDPWithDeviceMeshAndDTensor(DTensorTestBase):
    ...
    def test_dtensor_sharded_optim_load_state_dict(
        ...
        checkpoint.seek(0)
        load_ref_optim_state_dict = torch.load(checkpoint, weights_only=False)
    ...
    def test_dtensor_sharded_model_load_state_dict(
        ...
        checkpoint.seek(0)
        load_ref_state_dict = torch.load(checkpoint, weights_only=False)
