...
class TestDummyModel(torch.nn.Module):
    def __init__(self) -> None:
        super().__init__()
        torch.manual_seed(0)
        self.net1 = nn.Sequential(nn.Linear(8, 4), nn.ReLU())
        self.net2 = nn.Sequential(nn.Linear(4, 4), nn.ReLU())
        self.net3 = self.net2
        self.random_parameter = nn.Parameter(torch.Tensor(4))
...
class TestFSDPStateDict(FSDPTest):
    ...
    def _get_simple_nested_model(
        ...
        if wrap:
            lin1 = nn.Linear(4, 4, bias=False).cuda()
            lin2 = nn.Linear(4, 4, bias=False).cuda()
            if checkpoint_wrap:
    ...
    def _get_non_fsdp_root_module(self, *fsdp_args, wrap=True, **fsdp_kwargs):
        class FSDPContainer(nn.Module):
            def __init__(self, fsdp_1, fsdp_2):
                super().__init__()
                self.non_fsdp_lin = nn.Linear(4, 4, bias=False).cuda()
                self.fsdp_1 = fsdp_1
                self.fsdp_2 = fsdp_2
    ...
    def test_basic_save_and_load_state_dict(
        ...
        for model_call in [
            ...
            inp = torch.randn((3, 4), device=device)
            if fp16:
                inp = inp.half()
    ...
    def test_save_and_load_after_forward_state_dict(
        ...
        for _ in range(2):
            inp = torch.randn(1, 4, device=torch.cuda.current_device())
            output = model(*inp)
            loss = output.sum()
    ...
    def _dist_train(
        ...
        in_data = torch.rand(4, 4, requires_grad=True, device=torch.device("cuda"))
        for _ in range(2):
            out = model(in_data)
            out.sum().backward()
            optim.step()
    ...
    def test_state_dict_load_into_local_module(
        ...
        if not fsdp_root:
            in_data = torch.randn(
                1, 4, requires_grad=True, device=torch.device("cuda")
            )
        else:
            in_data = torch.rand(4, 4, requires_grad=True, device=torch.device("cuda"))
        for _ in range(2):
    ...
    def test_torch_save_load(self):
        ...
        with FSDP.state_dict_type(model, StateDictType.LOCAL_STATE_DICT):
            ...
            with torch.serialization.safe_globals(
                ...
                state_dict_saved = torch.load(checkpoint, weights_only=True)
