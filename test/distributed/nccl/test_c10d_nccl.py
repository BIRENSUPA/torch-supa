...
def is_qemu_cpu_model():
    ...
    for line in output.splitlines():
        if line.lower().startswith("model name") and "qemu" in line.lower():
            return True
    return False
...
class ProcessGroupNCCLInitTest(MultiProcessTestCase):
    ...
    def _init_process_group(self, **kwargs):
        ...
        print(f"init_process_group: {self.rank} {self.world_size} {self.file_name}")
    ...
    @requires_nccl()
    @skip_if_lt_x_gpu(1)
    @pytest.mark.skip(reason="MLFW-10004: random failed test")
    @pytest.mark.regression
    def test_init_wo_backend_str(self):
        self._init_process_group(device_id=self.device)
        print(f"test_init_wo_backend_str: {self.rank} {self.world_size} {self.file_name}")
        x = torch.empty(1, device=self.device)
        print(f"test_init_wo_backend_str2: {self.rank} {self.world_size} {self.file_name}")
        c10d.all_reduce(x)
...
class ProcessGroupNCCLGroupTest(MultiProcessTestCase):
    ...
    def test_nccl_dist_backend_error(self):
        raise SkipTest("BR200 ERROR: DistBackednError not raised")
        store = c10d.FileStore(self.file_name, self.world_size)
    ...
    def test_abort_pg(self):
        raise SkipTest("BR200 ERROR: SQ/LSC hang")
    ...
    @skip_if_rocm_multiprocess
    @pytest.mark.regression
    @pytest.mark.skip(reason="MLFW-10579. not support.")
    ...
    def test_set_nccl_pg_timeout(self, backend):
        # TODO:(bccl) remove timeout wa
        timeout_scale = 1
        if is_qemu_cpu_model():
            timeout_scale = 10
        ...
        opts = dict(
            ...
            rank=self.rank,
            world_size=self.world_size,
            timeout=timedelta(seconds=123 * timeout_scale),
        )
        dist.init_process_group(**opts)
        pg = dist.distributed_c10d._get_default_group()
        pg.allreduce(torch.rand(10).cuda(self.rank))
        self._check_nccl_timeout(timedelta(seconds=123 * timeout_scale))
        pg._get_backend(torch.device(f"cuda:{self.rank}"))._set_default_timeout(
            timedelta(seconds=23 * timeout_scale)
        )
        self._check_nccl_timeout(timedelta(seconds=23 * timeout_scale))
        pg.allreduce(torch.rand(10).cuda(self.rank))
        c10d.distributed_c10d._set_pg_timeout(timedelta(seconds=252 * timeout_scale), pg)
        self._check_nccl_timeout(timedelta(seconds=252 * timeout_scale))
    ...
    def test_extend_nccl_pg_timeout(self, backend):
        ...
        self.assertTrue(bankend._verify_work_timeout(w, timedelta(seconds=123 * timeout_scale)))
        w.wait()
        bankend._set_default_timeout(timedelta(seconds=3 * timeout_scale))
        if self.rank == 0:
            ...
            w = pg.allreduce(torch.rand(10).cuda(self.rank))
            self.assertTrue(bankend._verify_work_timeout(w, timedelta(seconds=3 * timeout_scale)))
            w.wait()
        else:
            dist.distributed_c10d._add_ephemeral_timeout_for_all_pgs(
                timedelta(seconds=10 * timeout_scale)
            )
            w1 = pg.allreduce(torch.rand(10).cuda(self.rank))
            w2 = pg.allreduce(torch.rand(5).cuda(self.rank))
            self.assertTrue(bankend._verify_work_timeout(w1, timedelta(seconds=13 * timeout_scale)))
            self.assertTrue(bankend._verify_work_timeout(w2, timedelta(seconds=13 * timeout_scale)))
            w1.wait()
            dist.distributed_c10d._add_ephemeral_timeout_for_all_pgs(
                timedelta(seconds=5 * timeout_scale)
            )
            ...
            w = pg.allreduce(torch.rand(10).cuda(self.rank))
            self.assertTrue(bankend._verify_work_timeout(w, timedelta(seconds=8 * timeout_scale)))
            w.wait()
    ...
    @pytest.mark.regression
    @pytest.mark.skip(reason="BCCL error: check jira MLFW-9796")
    ...
    @requires_nccl_version((2, 18), "Need NCCL 2.18+ for ncclCommSplit")
    @skip_but_pass_in_sandcastle_if(not TEST_MULTIGPU, "NCCL test requires 2+ GPUs")
    @pytest.mark.skip(reason="fail on br299")
    @pytest.mark.regression
    ...
    def test_subgroup_p2p(self, eager_init: bool):
        if eager_init:
            raise SkipTest("BR200 ERROR: timeout")
        store = c10d.FileStore(self.file_name, self.world_size)
        device = torch.device(f"cuda:{self.rank % torch.cuda.device_count()}")
        c10d.init_process_group(
            "nccl",
...
class DistributedDataParallelTest(
    ...
    @skip_if_lt_x_gpu(2)
    @pytest.mark.regression
    @pytest.mark.skip(reason="BR200: Timeout")
    ...
    @skip_if_lt_x_gpu(2)
    @pytest.mark.regression
    @pytest.mark.skip(reason="BR200: Hook result does not sync before compare with golden")
    ...
    def test_ddp_packed_sequence(self):
        raise SkipTest("BR200 ERROR: sudnn status not supported")
        """
    ...
    def test_channels_last_contig(self):
        raise SkipTest("BR200 ERROR: Qemu terminated")
        process_group = self._get_process_group()
        device = torch.device(f"cuda:{self.rank}")
    ...
    def test_ddp_complex_params(self):
        raise SkipTest("BR200 ERROR: no fft support")
...
class WorkHookTest(MultiProcessTestCase):
    ...
    def test_on_completion_hook_with_ddp(self):
        ...
        nlayers = 2
        net = nn.Sequential(
            *[nn.Linear(64, 64, bias=False) for _ in range(nlayers)]
        ).to(self.rank)
        ...
        x = torch.zeros(2, 64).cuda(self.rank)
        ddp(x).sum().backward()
...
class CommTest(test_c10d_common.AbstractCommTest, MultiProcessTestCase):
    ...
    def test_broadcast_coalesced_nccl(self):
        raise SkipTest("BR200 ERROR: double not supported")
        store = c10d.FileStore(self.file_name, self.world_size)
        c10d.init_process_group(
            backend="nccl", store=store, rank=self.rank, world_size=self.world_size
        )
    ...
    def test_all_reduce_coalesced_nccl_float8_errors(self):
        ...
        with self.assertRaisesRegex(
            ...
            "Float8 dtypes are not currenlty supported for BCCL reductions",
        ):
            torch.distributed.all_reduce_coalesced(tensors, group=process_group)
    ...
    @skip_if_rocm_multiprocess
    @pytest.mark.regression
    @pytest.mark.skip(reason="BR200: BCCL Fatal Error")
    ...
    def test_pass_nccl_options_config(self):
        raise SkipTest("BR200 ERROR: int() argument 'NoneType'")
        pg_opts = c10d.ProcessGroupNCCL.Options()
    ...
    @with_dist_debug_levels(levels=["DETAIL"])
    @pytest.mark.regression
    @pytest.mark.skip(reason="BR200: BCCL Internal Error: ring 0 does not loop back to start (1 != 0)")
...
class SparseCollective(MultiProcessTestCase):
    ...
    def test_ddp_set_sparse_metadata(self):
        raise SkipTest("BR200 ERROR: rank distribute failed")
        store = dist.FileStore(self.file_name, self.world_size)
        dist.init_process_group(
            "nccl",
            world_size=self.world_size,
...
class NCCLTraceTest(NCCLTraceTestBase):
    ...
    @requires_nccl()
    @skip_but_pass_in_sandcastle_if(not TEST_MULTIGPU, "NCCL test requires 2+ GPUs")
    # @parametrize("timing_enabled", [True, False]) disable timing on BR200 cmodel
    ...
    def test_long(self):
        ...
        first = t[0]
        last = t[-1]
        self.assertEqual(last["profiling_name"], "bccl:all_reduce")
        self.assertEqual(last["state"], "completed")
        self.assertIn("test_c10d_nccl.py", str(last["frames"]))
        self.assertEqual(last["input_sizes"], ((3, 4),))
        self.assertEqual(last["input_dtypes"], ["Float"])
    ...
    def test_barrier_profiling(self):
        ...
        first = t[0]
        last = t[-1]
        self.assertEqual(first["profiling_name"], "bccl:all_reduce_barrier")
        self.assertEqual(last["profiling_name"], "bccl:all_reduce")
        dist.destroy_process_group()
    ...
    def test_trace_while_active(self, timing_enabled, only_active):
        ...
        with torch.cuda.device(device):
            ...
            if not only_active:
                if self.rank == 0:
                    self.assertEqual(t[-1]["profiling_name"], "bccl:all_reduce")
    ...
    def test_batched_send_recv(self, op_sizes_per_coalesce, timing_enabled):
        ...
        for seq in range(num_coalesced_ops):
            ...
            for p2p_op_idx, input_sizes in zip(
                ...
                profiling_name = (
                    "bccl:recv 0<-1" if self.rank == 0 else "bccl:send 1->0"
                )
                self.assertEqual(
            ...
            expected_record_id += 1
            self.assertEqual(
                t["entries"][coalesced_op]["profiling_name"], "bccl:coalesced"
            )
    ...
    def test_individual_send_recv(self, op_sizes, timing_enabled):
        ...
        for seq in range(num_repeats * ops_per_repeat):
            ...
            profiling_name = "bccl:recv 0<-1" if self.rank == 0 else "bccl:send 1->0"
    ...
    def test_coalescing_manager_collective(self, timing_enabled):
        ...
        self.assertEqual(
            t["entries"][0]["profiling_name"], "bccl:reduce_scatter_tensor_coalesced"
        )
