...
class TestAdaptiveMaxPool3dMethod:
    @pytest.mark.parametrize("input_shape, output_shape, dtype", params)
    def test_adaptive_maxpool3d(self, input_shape, output_shape, dtype):
        ...
        adaptive_maxpool3d_cpu = nn.AdaptiveMaxPool3d(
            (output_shape[2], output_shape[3], output_shape[4])
        )
        y_cpu = adaptive_maxpool3d_cpu(x_cpu.to(float))
        y_cpu.backward(cpu_grad)

        adp_avg_pool_supa = adaptive_maxpool3d_cpu.to(supa_device)
