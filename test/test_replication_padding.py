...
params_2d = [
    ...
    pytest.param((2, 1023, 511), torch.float32, marks=[pytest.mark.gcuStress]),
    pytest.param((2, 1025, 513), torch.float32, marks=[pytest.mark.gcuStress]),
    pytest.param((2, 1028, 2135), torch.float32, marks=[pytest.mark.gcuStress]),
]
...
params_3d = [
    ...
    pytest.param((2, 1023, 5, 511), torch.float32, marks=[pytest.mark.gcuStress]),
    pytest.param((2, 1025, 5, 513), torch.float32, marks=[pytest.mark.gcuStress]),
    pytest.param((2, 1028, 5, 2135), torch.float32, marks=[pytest.mark.gcuStress]),
]
...
class TestReplicationPad1d:
    ...
    def test_replicationpad1d(self, shape, dtype, pad):
        x_cpu, x_supa = create_random_tensor(shape, dtype=dtype, requires_grad=True)

        y_cpu = nn.functional.pad(x_cpu, pad, mode="replicate")
        y_supa = nn.functional.pad(x_supa, pad, mode="replicate")
...
class TestReplicationPad2d:
    ...
    def test_replicationpad2d(self, shape, dtype, pad):
        x_cpu, x_supa = create_random_tensor(shape, dtype=dtype, requires_grad=True)
    ...
    @pytest.mark.parametrize("shape, dtype", params_2d)
    @pytest.mark.parametrize("pad", pad_2d)
    def test_replicationpad2d_out(self, shape, dtype, pad):
        ...
        y_cpu = torch._C._nn.replication_pad2d(x_cpu, pad, out=y_cpu)
        y_supa = torch._C._nn.replication_pad2d(x_supa, pad, out=y_supa)

        assert_allclose(y_cpu, y_supa, rtol=0, atol=0)


class TestReplicationPad3d:

    @pytest.mark.parametrize("shape, dtype", params_3d)
    @pytest.mark.parametrize("pad", pad_3d)
    def test_replicationpad3d(self, shape, dtype, pad):
        x_cpu, x_supa = create_random_tensor(shape, dtype=dtype, requires_grad=True)
    ...
    @pytest.mark.parametrize("shape, dtype", params_3d)
    def test_replicationpad3d_out(self, shape, dtype):
        x_cpu, x_supa = create_random_tensor(shape, dtype=dtype, requires_grad=False)
