...
class TestHardSigmoid:
    ...
    def test_hardsigmoid(self, input_shape, dtype, requires_grad):
        cpu_input, supa_input = create_random_tensor(
            input_shape, dtype=dtype, requires_grad=requires_grad
        )

        cpu_out = torch.nn.functional.hardsigmoid(cpu_input)
        supa_out = torch.nn.functional.hardsigmoid(supa_input)
