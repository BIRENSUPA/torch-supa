...
br200_shapes = [
    ...
    pytest.param((1023, 510), marks=[pytest.mark.gcuStress]),
    pytest.param((1025, 514), marks=[pytest.mark.gcuStress]),
    pytest.param((1028, 2136), marks=[pytest.mark.gcuStress]),
]

dtypes = [torch.float32, torch.bfloat16, torch.float16]
RTOL = {torch.float32: 1.3e-6, torch.bfloat16: 1.6e-2, torch.float16: 1e-3}
ATOL = {torch.float32: 1e-5, torch.bfloat16: 1e-2, torch.float16: 1e-5}
...
class TestGlu:
    ...
    def test_glu(self, input_shape, dtype, requires_grad):
        cpu_input, supa_input = create_random_tensor(
            input_shape, dtype=dtype, requires_grad=requires_grad
        )

        cpu_out = torch.nn.functional.glu(cpu_input)
        supa_out = torch.nn.functional.glu(supa_input)
