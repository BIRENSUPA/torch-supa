...
def test_tensor(shape):
    ...
    y_supa = x_supa.normal_(mean=1.0, std=2.0)
    ...
    print(median_val)
    assert abs(median_val - 1.0) < 0.5


@pytest.mark.parametrize("shape", intern_vl_params)
@pytest.mark.parametrize("seed", SEED)
def test_normal(shape, seed):
    _, x_supa = create_random_tensor(
        shape,
        dtype=torch.float32,
        requires_grad=False,
    ...
    output0.normal_(mean=1.0, std=2.0)
    torch.manual_seed(seed)
    output1 = x_supa.clone()
    output1.normal_(mean=1.0, std=2.0)
    assert_allclose(output0, output1, rtol=0, atol=0)

    torch.manual_seed(seed)
    output0 = x_supa.normal_(mean=1.0, std=2.0)
    torch.manual_seed(seed)
    output1 = x_supa.normal_(mean=1.0, std=2.0)
    assert_allclose(output0, output1, rtol=0, atol=0)


@pytest.mark.parametrize("shape", intern_vl_params)
@pytest.mark.parametrize("seed", SEED)
def test_normal_bf16(shape, seed):
    _, x_supa = create_random_tensor(
        shape,
...
@pytest.mark.parametrize("shape", intern_vl_params)
@pytest.mark.parametrize("seed", SEED)
def test_normal_fp16(shape, seed):
    _, x_supa = create_random_tensor(
        shape,
        dtype=torch.float16,
        requires_grad=False,
