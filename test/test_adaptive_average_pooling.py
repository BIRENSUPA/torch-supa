...
params = [
    ...
    pytest.param(
        (31, 63, 13, 13), (31, 63, 5, 5), torch.float32, marks=[pytest.mark.gcuStress]
    ),
    pytest.param(
        (15, 61, 15, 15), (15, 61, 7, 7), torch.float32, marks=[pytest.mark.gcuStress]
    ),
    pytest.param(
...
class TestAdaptiveAvgPool2dMethod:
    @pytest.mark.parametrize("input_shape, output_shape, dtype", params)
    def test_adaptive_avgpool2d(self, input_shape, output_shape, dtype):
        ...
        adaptive_avgpool2d_cpu = nn.AdaptiveAvgPool2d(
            (output_shape[2], output_shape[3])
        )
        y_cpu = adaptive_avgpool2d_cpu(x_cpu)
        y_cpu.backward(cpu_grad)

        adp_avg_pool_supa = adaptive_avgpool2d_cpu.to(supa_device)
