...
def test_cauchy(shape, seed):
    ...
    torch.manual_seed(seed)
    output0 = x_supa.cauchy_()
    torch.manual_seed(seed)
    output1 = x_supa.cauchy_()
    assert_allclose(output0, output1, rtol=0, atol=0)


@pytest.mark.parametrize("shape", intern_vl_params)
@pytest.mark.parametrize("seed", SEED)
def test_cauchy_bf16(shape, seed):
    _, x_supa = create_random_tensor(
        shape,
...
@pytest.mark.parametrize("shape", intern_vl_params)
@pytest.mark.parametrize("seed", SEED)
def test_cauchy_fp16(shape, seed):
    _, x_supa = create_random_tensor(
        shape,
        dtype=torch.float16,
        requires_grad=False,
