...
class TestHardSwish:
    ...
    def test_hardswish(self, input_shape, dtype, requires_grad):
        ...
        for i in range(0, length):
            if torch.abs(cpu_input.view(-1)[i]) == 3:
                cpu_input.view(-1)[i].add_(0.5)
                supa_input.view(-1)[i].add_(0.5)

        cpu_input.requires_grad_(requires_grad)
        supa_input.requires_grad_(requires_grad)

        cpu_out = torch.nn.functional.hardswish(cpu_input)
        supa_out = torch.nn.functional.hardswish(supa_input)
