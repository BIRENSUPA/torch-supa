...
class TestAvgPool2dMethod:

    @pytest.mark.parametrize(
        "input_shape, output_shape, dtype, kernel, stride", br200_params
    )
    def test_avgpool2d(self, input_shape, output_shape, dtype, kernel, stride):
        ...
        avgpool2d_cpu = nn.AvgPool2d(kernel, stride=stride)
        y_cpu = avgpool2d_cpu(x_cpu)
        y_cpu.backward(cpu_grad)

        avg_pool_supa = avgpool2d_cpu.to(supa_device)
    ...
    def test_avgpool2d_pad(
        ...
        avgpool2d_cpu = nn.AvgPool2d(kernel, stride=stride, padding=padding)
        y_cpu = avgpool2d_cpu(x_cpu)
        y_cpu.backward(cpu_grad)
    ...
    def test_avgpool2d_exclude_supa_kernel(
        self,
        input_shape,
        output_shape,
        dtype,
