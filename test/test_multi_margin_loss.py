...
br200_params = [
    ...
    pytest.param((32, 8), (32), torch.float32, marks=[pytest.mark.gcuStress]),
    pytest.param((32, 32), (32), torch.float32, marks=[pytest.mark.gcuStress]),
    pytest.param((32, 30), (32), torch.float32, marks=[pytest.mark.gcuStress]),
]
...
class TestMultiMarginLoss:
    @pytest.mark.parametrize("shape, target_shape, dtype", br200_params)
    def test_multi_margin_loss_fwd(self, shape, target_shape, dtype):
    ...
    @pytest.mark.parametrize("shape, target_shape, dtype", br200_params)
    def test_multi_margin_loss_bwd(self, shape, target_shape, dtype):
        ...
        assert_allclose(cpu_grad.to(dtype), supa_grad, atol=5e-3, rtol=RTOL[dtype])
