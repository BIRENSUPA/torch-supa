...
class TestHardShrink:
    ...
    def test_hardshrink(self, input_shape, dtype, requires_grad):
        cpu_input, supa_input = create_random_tensor(
            input_shape, dtype=dtype, requires_grad=requires_grad
        )

        cpu_out = torch.nn.functional.hardshrink(cpu_input)
        supa_out = torch.nn.functional.hardshrink(supa_input)
