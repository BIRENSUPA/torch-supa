...
# Helper to exclude unsupported dtypes for SUPA device
def _dtypes_excluding_double_and_complex128(*extra_dtypes):
    """Get all types excluding float64 and complex128."""
    excluded = {torch.float64, torch.complex128}
    all_types = all_types_and_complex_and(*extra_dtypes)
    return [dt for dt in all_types if dt not in excluded]
...
class TestTorchDlPack(TestCase):
    ...
    @onlyNativeDeviceTypes
    @dtypes(
        *_dtypes_excluding_double_and_complex128(
            torch.half,
            torch.bfloat16,
            torch.bool,
            torch.uint16,
    ...
    @skipMeta
    @onlyCUDA
    @dtypes(*_dtypes_excluding_double_and_complex128(torch.half, torch.bfloat16, torch.bool))
    ...
    @onlyCUDA
    @skipCUDAIfRocm
    @pytest.mark.skip(reason="AssertionError: True is not false")
    ...
    @skipMeta
    @onlyNativeDeviceTypes
    @dtypes(*_dtypes_excluding_double_and_complex128(torch.half, torch.bfloat16))
    ...
    def test_dlpack_export_requires_grad(self):
        ...
        with self.assertRaisesRegex(err, r"require gradient"):
            x.__dlpack__()
    ...
    def test_dlpack_export_is_conj(self):
        ...
        if torch_version_ge(2, 9, 0):
            err = BufferError
        with self.assertRaisesRegex(err, r"conjugate bit"):
            y.__dlpack__()
    ...
    def test_dlpack_export_non_strided(self):
        ...
        if torch_version_ge(2, 9, 0):
            err = BufferError
        with self.assertRaisesRegex(err, r"strided"):
            y.__dlpack__()
