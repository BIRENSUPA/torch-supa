...
RTOL = {torch.float32: 1.3e-6, torch.bfloat16: 6e-1, torch.float16: 1e-2}
ATOL = {torch.float32: 1e-5, torch.bfloat16: 1e-2, torch.float16: 3e-1}

cpu_device = torch.device("cpu")
supa_device = torch.device("supa")


class TestLogcumsumexpMethod:
    ...
    def test_logaddexp(self, shape, dtype):
        ...
        output_cpu = torch.logcumsumexp(in_cpu, dim=0)
        output_supa = torch.logcumsumexp(in_supa, dim=0)
