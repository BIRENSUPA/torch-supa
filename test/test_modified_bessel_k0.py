...
class TestModifiedBesselK0Method:

    @pytest.mark.parametrize("shape", br200_shape)
    @pytest.mark.parametrize("dtype", dtypes)
    def test_modified_bessel_k0(self, shape, dtype):
        in_cpu, in_supa = create_random_tensor(shape, dtype=dtype, requires_grad=False)

        output_cpu = torch.special.modified_bessel_k0(in_cpu)
        output_supa = torch.special.modified_bessel_k0(in_supa)
