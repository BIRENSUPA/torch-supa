...
def test_tensor(shape):
    ...
    y_supa = x_supa.log_normal_(mean=1.0, std=2.0)

    y_cpu = y_supa.cpu()
    y_cpu_sorted = torch.sort(y_cpu)
    median_val = y_cpu_sorted.values.median().item()

    median_golden = 2.7182818

    print(median_val)
    print(median_golden)
    assert abs(median_val - median_golden) < 0.5


@pytest.mark.parametrize("shape", intern_vl_params)
@pytest.mark.parametrize("seed", SEED)
def test_log_normal(shape, seed):
    _, x_supa = create_random_tensor(
        shape,
        dtype=torch.float32,
        requires_grad=False,
    ...
    output0.log_normal_(mean=1.0, std=2.0)
    torch.manual_seed(seed)
    output1 = x_supa.clone()
    output1.log_normal_(mean=1.0, std=2.0)
    assert_allclose(output0, output1, rtol=0, atol=0)

    torch.manual_seed(seed)
    output0 = x_supa.log_normal_(mean=1.0, std=2.0)
    torch.manual_seed(seed)
    output1 = x_supa.log_normal_(mean=1.0, std=2.0)
    assert_allclose(output0, output1, rtol=0, atol=0)


@pytest.mark.parametrize("shape", intern_vl_params)
@pytest.mark.parametrize("seed", SEED)
def test_log_normal_bf16(shape, seed):
    _, x_supa = create_random_tensor(
        shape,
...
@pytest.mark.parametrize("shape", intern_vl_params)
@pytest.mark.parametrize("seed", SEED)
def test_log_normal_fp16(shape, seed):
    _, x_supa = create_random_tensor(
        shape,
        dtype=torch.float16,
        requires_grad=False,
