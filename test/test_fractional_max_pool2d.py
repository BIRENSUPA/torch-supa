...
class TestNNMethod:
    ...
    def test_fractional_maxpool2d_fwd(self, shape, kernel, output_ratio, dtype):

        cpu_input, supa_input = create_random_tensor(
            shape, dtype=dtype, requires_grad=False
        )

        cpu_output = F.fractional_max_pool2d(
            cpu_input, kernel_size=kernel, output_ratio=output_ratio
        )
        supa_output = F.fractional_max_pool2d(
            supa_input, kernel_size=kernel, output_ratio=output_ratio
        )

        assert_allclose(cpu_output, supa_output, atol=ATOL[dtype], rtol=RTOL[dtype])

    @pytest.mark.parametrize("shape", br200_shapes)
    @pytest.mark.parametrize("kernel", kernel)
    @pytest.mark.parametrize("output_ratio", output_ratio)
    @pytest.mark.parametrize("dtype", dtypes)
    def test_fractional_maxpool2d_bwd(self, shape, kernel, output_ratio, dtype):
        cpu_input, supa_input = create_random_tensor(
    ...
    @pytest.mark.parametrize("output_ratio", output_ratio)
    @pytest.mark.parametrize("dtype", dtypes)
    def test_fractional_maxpool2d_fwd_br200(self, shape, kernel, output_ratio, dtype):
        self.test_fractional_maxpool2d_fwd(
    ...
    @pytest.mark.parametrize("output_ratio", output_ratio)
    @pytest.mark.parametrize("dtype", dtypes)
    def test_fractional_maxpool2d_bwd_br200(self, shape, kernel, output_ratio, dtype):
        self.test_fractional_maxpool2d_bwd(
            shape=shape, kernel=kernel, output_ratio=output_ratio, dtype=dtype
        )
