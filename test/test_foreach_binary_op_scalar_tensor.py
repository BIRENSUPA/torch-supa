...
class TestForeachBinaryMethod:
    ...
    def test_foreach_binary_op_scalar_tensor(self, shape, func_type, dtype):
        ...
        res_cpu = pt_func(in_cpu_list, scalar_cpu)
        res_supa = pt_func(in_supa_list, scalar_supa)
        assert_allclose(
    ...
    @pytest.mark.parametrize("shape", br200_shape)
    @pytest.mark.parametrize("dtype", dtypes)
    def test_foreach_pow_scalar_tensor(self, shape, dtype):
        in_cpu, in_supa = create_random_tensor(shape, dtype)
        in_cpu2, in_supa2 = create_random_tensor(shape, dtype)
        ...
        res_cpu = torch._foreach_pow(scalar_cpu, in_cpu_list)
        res_supa = torch._foreach_pow(scalar_supa, in_supa_list)
