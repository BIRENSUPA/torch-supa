...
设置是否打印 ``transfer_to_supa`` 日志，默认为 ``false``。当值为 ``ON``、``1``、``YES``、``TRUE`` 或 ``Y`` 时，不打印相关日志信息。
...
设置日志系统的后端实现。torch_supa 支持多种日志后端，可设置 ``glog``、 ``stdout``、 ``empty``， 默认使用 ``glog``。
...
设置日志文件的保存目录。日志系统会在此目录下创建日志文件，记录运行时信息，默认为 ``./logs``。
...
控制是否启用实时日志打印功能。启用后，日志会立即输出到控制台，便于调试和问题定位，默认为 ``false``。
在大规模训练场景下，建议关闭以提高性能。
...
设置实时日志的大小阈值，单位为 KB，默认为 12800。当日志文件大小超过此阈值时，会触发相应的处理机制。
...
控制是否启用信号处理功能，Release 模式默认为 ``false``。启用后，torch_supa 会捕获系统信号（如 ``SIGSEGV``、 ``SIGABRT`` 等），
...
控制是否立即刷新日志缓冲区，Release 模式默认为 ``false``。启用后，每条日志都会立即写入文件，确保日志完整性，
...
设置异步日志队列的大小，默认为 1048576。异步日志系统会将日志消息放入队列，由后台线程写入文件。
...
控制是否优先使用 PyTorch 原生算子实现，默认为 ``false``。默认情况下，torch_supa 会使用针对壁仞硬件优化的算子实现。
